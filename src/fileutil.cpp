...
QString FileUtil::getOpenFileName(QWidget* parent, const QString& caption, const QString& filter, QString* selectedFilter)
{
	while (1) {
		if (lastUsedOpenPath().isEmpty()) {
			setLastUsedOpenPath(defaultPath());
		}
...
QString FileUtil::getSaveFileName(QWidget* parent, const QString& caption, const QString& defaultFileName, const QString& filter, QString* selectedFilter)
{
	if (lastUsedSavePath().isEmpty()) {
		if (!lastUsedOpenPath().isEmpty()) {
			...
			setLastUsedSavePath(defaultPath());
		}
	}
...
QString FileUtil::getOpenAnyFileName(QWidget* parent)
{
	return FileUtil::getOpenFileName(parent, tr("Choose a file"), tr("All files (*)"));
}

QStringList FileUtil::getOpenAnyFileNames(QWidget* parent)
{
	return FileUtil::getOpenFileNames(parent, tr("Choose a file"), tr("All files (*)"));
}
