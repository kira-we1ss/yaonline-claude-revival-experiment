...
ContactListView::ContactListView(QWidget* parent)
	...
	, updateGroupExpandedStateEnabled_(true)
{
...
void ContactListView::setLargeIcons(bool b)
{
	if (largeIcons_ != b) {
		...
		emit largeIconsChanged(largeIcons_);
	}
}
...
void ContactListView::setShowIcons(bool b)
{
	if (showIcons_ != b) {
		showIcons_ = b;
		emit showIconsChanged(showIcons_);
	}
}
...
bool ContactListView::updateGroupExpandedStateEnabled() const
{
	return updateGroupExpandedStateEnabled_;
}

void ContactListView::setUpdateGroupExpandedStateEnabled(bool enabled)
{
	updateGroupExpandedStateEnabled_ = enabled;
}

void ContactListView::updateGroupExpandedState()
{
	if (!model() || !updateGroupExpandedStateEnabled_)
		return;
...
void ContactListView::updateContextMenu()
{
	...
	if (selectedIndexes().count() == 1) {
		contextMenu_ = createContextMenuFor(selectedIndexes());
		addContextMenuActions();
	}
}

ContactListItemMenu* ContactListView::createContextMenuFor(const QModelIndexList& indexes) const
{
	Q_ASSERT(!indexes.isEmpty());
	ContactListItemMenu* result = 0;
	ContactListItemProxy* item = itemProxy(indexes.first());
	if (item) {
		result = createContextMenuFor(item->item());
	}
	return result;
}
