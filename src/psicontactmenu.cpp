...
public:
	...
	void updateMenu(PsiContact* contact, ContactListModel* model)
	{
		if (isVisible())
			return;
		contact_ = contact;
		...
		int n = 0;
#ifdef YAPSI
		existingGroups_ = Ya::filterGroupNames(model->groupCache()->groups());
#else
		existingGroups_ = contact->account()->groupList();
		// existingGroups_.removeAll(tr("Hidden"));
#endif
		existingGroups_.sort();
		foreach(QString groupName, existingGroups_) {
			bool selected = contact->userListItem().groups().contains(groupName);
			QString displayName = groupName;
			if (displayName.isEmpty())
				displayName = PsiContact::generalGroupName();

			if (displayName == PsiContact::generalGroupName()) {
				bool inGeneralGroup = false;
				foreach(QString contactGroup, contact->userListItem().groups()) {
				...
				selected |= (inGeneralGroup || contact->userListItem().groups().isEmpty());
			}
			...
			addGroup(text, groupName, selected);
		}
...
private slots:
	...
	{
#ifdef YAPSI
		QString newGroupName = Ya::createUniqueName(FakeGroupContact::defaultGroupName(), existingGroups_);
		emit groupActivated(newGroupName);
#else
		while (contact_) {
			bool ok = false;
...
class InviteToGroupChatMenu : public QMenu
{
	Q_OBJECT
public:
	InviteToGroupChatMenu(const QString& title, QWidget* parent)
		: QMenu(title, parent)
	{
	}
...
public:
	...
	QAction* yaEmail_;
	QAction* fileUpload_;
	InviteToGroupChatMenu* inviteMenu_;
...
public:
		...
		, menu_(menu)
	{
		...
		inviteMenu_ = new InviteToGroupChatMenu(tr("Invite to"), menu_);
		connect(inviteMenu_, SIGNAL(inviteToGroupchat(PsiAccount*, QString)), SLOT(inviteToGroupchat(PsiAccount*, QString)));
		...
		menu->addAction(fileUpload_);
		if (GCMainDlg::mucEnabled()) {
			menu->addMenu(inviteMenu_);
		}
		menu->addSeparator();
...
private slots:
	...
	void updateActions()
	{
		...
		openHistory_->setEnabled(contact_->historyAvailable());
		delete profile;

		rename_->setEnabled(contact_->isRenameable());
		yaProfile_->setEnabled(contact_->isYaJid());
		yaPhotos_->setEnabled(contact_->isYaJid());
		fileUpload_->setEnabled(contact_->isEditable());
		...
		inviteMenu_->updateMenu(contact_);
		inviteMenu_->setEnabled(!inviteMenu_->isEmpty());
		groupMenu_->updateMenu(contact_, menu_->model());

		block_->setText(contact_->isBlocked() ?
		               tr("&Unblock") : tr("&Block"));

		disableMoodNotifications_->setChecked(!contact_->moodNotificationsEnabled());
	...
	{
		if (contact_) {
			contact_->setMoodNotificationsEnabled(!disableMoodNotifications_->isChecked());
		}
	}
	...
	void createGroup()
	{
		if (contact_) {
			while (1) {
				bool ok = false;
				QString newGroup = QInputDialog::getText(tr("Create New Group"), tr("Enter the new Group name:"), QLineEdit::Normal, QString(), &ok, 0);
				if (!ok)
					break;
