...
{
	PsiLogger::instance()->log(QString("VCardFactory::saveVCard(%1)").arg(j.bare()));
...
const VCard* VCardFactory::vcard(const Jid &j)
{
	...
	QDomDocument doc;
	VCard *vcard = new VCard;
	PsiLogger::instance()->log(QString("VCardFactory::vcard(%1) - from file").arg(j.bare()));
...
{
	PsiLogger::instance()->log(QString("VCardFactory::setVCard(%1)").arg(account->jid().bare()));
...
{
	PsiLogger::instance()->log(QString("VCardFactory::getVCard(%1)").arg(jid.bare()));
