...
void ChatEdit::initHighlighters()
{
	SpellCheckingTextEdit::initHighlighters();
	...
	QMetaObject::invokeMethod(this, "optionChanged", Qt::QueuedConnection,
	                          QGenericReturnArgument(),
	                          Q_ARG(QString, spellCheckEnabledOptionPath));
}

void ChatEdit::clear()
{
	SpellCheckingTextEdit::clear();
#ifdef YAPSI
	updateMargins();
#endif
}
...
/*	else if((e->key() == Qt::Key_Return || e->key() == Qt::Key_Enter) && !((e->modifiers() & Qt::ShiftModifier) || (e->modifiers() & Qt::AltModifier)) && option.chatSoftReturn)
		...
		SpellCheckingTextEdit::keyPressEvent(e);
		updateMargins();
	}
#endif
	else
...
bool ChatEdit::eventFilter(QObject* o, QEvent* e)
{
	...
	return SpellCheckingTextEdit::eventFilter(o, e);
}
...
void ChatEdit::contextMenuEvent(QContextMenuEvent *e) 
{
	...
	if (uploadFileAction_ && recentFilesMenu_ && typographyAction_ && emoticonsAction_ && checkSpellingAction_ && sendButtonEnabledAction_) {
		menu->addSeparator();
		menu->addAction(uploadFileAction_);
...
#ifdef YAPSI
void ChatEdit::setUploadFileAction(QAction* uploadFileAction)
{
	uploadFileAction_ = uploadFileAction;
}

void ChatEdit::setRecentFilesMenu(QMenu* recentFilesMenu)
{
	recentFilesMenu_ = recentFilesMenu;
}
