...
static const QIcon& getMainWindowIcon(MainWindowIconType type)
{
	...
	if (windowIcons.empty()) {
		QIcon green;
		green.addFile(":images/windowicon_green.png", QSize(16, 16));
		green.addFile(":images/windowicon_green32.png", QSize(32, 32));
		windowIcons << green;

		QIcon yellow;
		yellow.addFile(":images/windowicon_yellow.png", QSize(16, 16));
		yellow.addFile(":images/windowicon_yellow32.png", QSize(32, 32));
		windowIcons << yellow;

		QIcon red;
		red.addFile(":images/windowicon_red.png", QSize(16, 16));
		red.addFile(":images/windowicon_red32.png", QSize(32, 32));
		windowIcons << red;
	}
	return windowIcons[type];
}
#endif
	...
	, selfMoodRecreated_(false)
{
	...
	connect(psi_->contactList(), SIGNAL(accountStateChanged()), SLOT(accountStateChanged()));
	accountCountChanged();
	accountStateChanged();
...
void YaMainWin::recreateSelfMood()
{
	if (selfMoodRecreated_)
		return;

	if ((ui_.topStack->currentWidget() == ui_.selfWidgetsPage) && isVisible()) {
		selfMoodRecreated_ = true;
	}

	setUpdatesEnabled(false);

	XMPP::Status::Type oldStatus = ui_.selfMood->statusType();
	...
	ui_.selfMood->setStatusType(oldStatus);
	ui_.selfMood->setContactList(psi_->contactList());
...
bool YaMainWin::event(QEvent* event)
{
	if (event->type() == QEvent::WindowActivate ||
	    event->type() == QEvent::Show)
	{
		QTimer::singleShot(0, this, SLOT(recreateSelfMood()));
	}

	return YaOnlineMainWin::event(event);
}
...
{
	PsiAccount* account = this->account();
	...
	// 		if (account->isAvailable() && (account->isYaAccount() || account->isYaPddAccount()))
	// 			return account;
	//
	// foreach(PsiAccount* account, psi_->contactList()->sortedEnabledAccounts())
...
{
	return psi_->contactList()->defaultAccount();
	// if (psi_->contactList()->haveActiveAccounts())
	// 	foreach(PsiAccount* account, psi_->contactList()->sortedEnabledAccounts())
...
void YaMainWin::accountStateChanged()
{
	...
	MainWindowIconType mwit = statusType() == XMPP::Status::Offline ?
	                          MWIT_Offline :
	                          MWIT_Online;
	if (haveConnectingAccounts) {
		mwit = MWIT_Reconnecting;
	}
	const QIcon& icon = getMainWindowIcon(mwit);
...
{
	PsiLogger::instance()->log(QString("YaMainWin()::statusSelected()"));
...
void YaMainWin::statusSelectedHelper(XMPP::Status::Type statusType, bool _isManualStatus)
{
	...
	PsiLogger::instance()->log(QString("YaMainWin()::statusSelectedHelper(%1, %2, %3)")
	                           .arg(statusType).arg(mood)
	                           .arg(isManualStatus));
...
{
	PsiLogger::instance()->log(QString("YaMainWin()::statusSelected(%1)")
	                           .arg(statusType));
...
{
	PsiLogger::instance()->log(QString("YaMainWin()::statusSelectedManuallyHelper(%1)")
	                           .arg(statusType));
...
{
	PsiLogger::instance()->log(QString("YaMainWin()::setStatusOnline()"));
	statusSelected(XMPP::Status::Online);
}

void YaMainWin::setStatusDND()
{
	PsiLogger::instance()->log(QString("YaMainWin()::setStatusDND()"));
	statusSelected(XMPP::Status::DND);
}

void YaMainWin::setStatusOffline()
{
	PsiLogger::instance()->log(QString("YaMainWin()::setStatusOffline()"));
