...
QVariant YaContactListModel::data(const QModelIndex& index, int role) const
{
	...
	if (role == Qt::ForegroundRole) {
		...
		return QVariant(Ya::contactName(
		                    ContactListModel::data(index, role).toString(),
		                    ContactListModel::data(index, ContactListModel::JidRole).toString()
		                ));
	}
...
QModelIndexList YaContactListModel::indexesFor(const QMimeData* data) const
{
	...
	foreach(YaContactListModelSelection::Contact contact, selection.contacts()) {
		...
		if (!psiContact) {
			foreach(PsiContact* c, additionalContacts()) {
				if (c->jid().compare(contact.jid, false) && c->account() == account) {
					psiContact = c;
					break;
				}
			}
...
#ifndef YAPSI
		if (groups.isEmpty() && !psiContact->isFake() && psiContact->inList()) {
#else
		if (psiContact) {
#endif
	...
	if (performRemove && !toRemove.isEmpty()) {
		...
		while (it.hasNext()) {
			...
			foreach(QString group, psiContact->groups()) {
				if (psiContact->isFake() && virtualUnremovableGroups().contains(group)) {
					psiContact = 0;
					break;
				}
			}
...
void YaContactListModel::initializeModel()
{
...
QStringList YaContactListModel::virtualGroupList() const
{
	QStringList result;
...
QStringList YaContactListModel::virtualUnremovableGroups()
{
	QStringList result;
...
void YaContactListModel::protectVirtualUnremovableGroups()
{
	protectVirtualUnremovableGroups(virtualUnremovableGroups());
}

void YaContactListModel::protectVirtualUnremovableGroups(QStringList endangeredGroups)
{
 ...
 */
QStringList YaContactListModel::contactGroupsLostByRemove(PsiContact* contact, const QMimeData* data) const
{
	Q_ASSERT(contact);
	Q_ASSERT(data);
	QStringList result;
	...
	YaContactListModelOperationList operations = removeOperationsFor(data);
	Q_ASSERT(operations.action() == YaContactListModelOperationList::Remove);
	foreach(const YaContactListModelOperationList::ContactOperation& op, operations.operations()) {
		if (op.contact != contact)
			continue;

		foreach(const YaContactListModelOperationList::Operation& o, op.operations) {
...
PsiContact* YaContactListModel::addFakeTempContact(const XMPP::Jid& jid, PsiAccount* account)
{
	Q_ASSERT(contactList() && contactList()->accountsLoaded());
	if (!contactList() || !contactList()->accountsLoaded())
		return 0;
	...
	u.setJid(jid.bare());
	u.setInList(false);
	u.setAvatarFactory(account->avatarFactory());
