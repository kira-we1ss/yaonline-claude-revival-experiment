...
bool YaPopupNotification::notify(int id, PsiEvent* event, int soundType)
{
	...
	if (event->type() == PsiEvent::Mood) {
		...
		if (contact) {
			...
			PsiNotifier::instance()->moodChanged(event->account(), contact->jid(), contactName, moodEvent->mood());
		}
		return true;
	}
	...
	if (PsiNotifier::instance()->isNative()) {
		return true;
	}
...
void YaPopupNotification::openNotify(int id, PsiAccount* account, const XMPP::Jid& jid, ActivationType activationType)
{
	...
	if (account) {
		if (activationType == UserAction) {
			...
			account->actionOpenPassiveChatSpecific(jid);
		}
		else {
