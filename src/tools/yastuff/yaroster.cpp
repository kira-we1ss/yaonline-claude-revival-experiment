...
private slots:
	void linkActivated(const QString& link)
	{
		...
		if (link == "yachat://add-contact") {
			...
			DesktopUtil::openYaUrl("http://my.ya.ru");
		}
...
public:
	...
	void initButtons()
	{
		...
		{
			addButton_ = new YaRosterAddContactToolButton(this);
			addButton_->init();
			addButton_->setRoster(yaRosterTab_->yaRoster());
...
private:
	...
	YaRosterAddContactToolButton* addButton_;
...
QPair<PsiAccount*, XMPP::Jid> YaRosterContactsTab::addContactHelper(const QString& text, PsiAccount* explicitAccount, const QStringList& groups, const QString& _desiredName)
{
	QString c = text.trimmed();
	QString desiredName = _desiredName;

	PsiContactList* contactList = this->contactList();
	PsiCon* controller = contactList ? contactList->psi() : 0;
	if (!contactList || !controller)
		return QPair<PsiAccount*, XMPP::Jid>();

	QString transportJid = controller->yaTransportManager()->transportJid(c);
	if (!transportJid.isEmpty()) {
		c = transportJid;
	}
	...
	Jid jid;
	if (controller->contactUpdatesManager()->notifier()->isRegisteredJid(c)) {
		jid = c;
		desiredName = controller->contactUpdatesManager()->notifier()->desiredName(c);
	}
	else {
		QRegExp rx("[@/]");
		if (rx.indexIn(c) == -1) {
	...
	foreach(PsiAccount* account, contactList->sortedEnabledAccounts()) {
		if (!account->isAvailable())
			continue;
		if (Ya::isYandexTeamJid(jid) && Ya::isYandexTeamJid(account->jid())) {
			acc = account;
			break;
		}
...
void YaRosterContactsTab::vcardFinished()
{
	...
	if (contact) {
		if (controller && controller->contactUpdatesManager()->notifier()->isRegisteredJid(j->jid().bare())) {
			contact->setName(controller->contactUpdatesManager()->notifier()->desiredName(j->jid().bare()));

			foreach(const YaTransport* transport, controller->yaTransportManager()->availableTransports()) {
				const_cast<YaTransport*>(transport)->setConnected(false);
				const_cast<YaTransport*>(transport)->setConnected(true);
			}
		}
		else if (contact->name() == j->jid().bare()) {
...
{
	updateFilterMode();
	proxyModel_->setFilterRegExp(Ya::contactListFilterRegExp(text));
}
...
void YaRoster::setContactList(PsiContactList* contactList)
{
	...
	contactList_->setContactListModel(contactListModel());
}
...
QString YaRoster::confirmationData(PsiContact* contact, QMimeData* _selection) const
{
	...
	if (model && !indexes.isEmpty()) {
		QMimeData* selection = model->mimeData(indexes);
		YaContactListModelSelection* yaContactListModelSelection = dynamic_cast<YaContactListModelSelection*>(selection);
		if (yaContactListModelSelection) {
			selectionData = QCA::Base64().arrayToString(yaContactListModelSelection->data(yaContactListModelSelection->mimeType()));
		}
	}
	...
	if (model && !indexes.isEmpty()) {
		...
		if (!removeConfirmed) {
			...
			if (contactNames.count() > 1 || contactListSelection.groups().count()) {
				...
				if (contactListSelection.groups().count() > 1 && !tmp.count()) {
					msg = tr("This will permanently remove:<br>"
					         "%n groups from your contact list.",
					         0,
			...
			if (indexes.count() == 1 && model->indexType(indexes.first()) == ContactListModel::GroupType) {
				QModelIndex realGroupIndex = indexes.first();
				QModelIndex proxyGroupIndex = contacts_->contactListView()->proxyIndex(realGroupIndex);
				int totalContacts = proxyGroupIndex.model()->rowCount(proxyGroupIndex);

				if (YaContactListModel::virtualUnremovableGroups().contains(contactListSelection.groups().first().fullName)) {
				...
				YaContactListModelSelection::Contact c = contactListSelection.contacts().first();
...
{
	YaContactListModelSelection selection(0);
...
void YaRoster::addSelection(QMimeData* data)
{
	...
	YaContactListModelSelection::Contact contact = selection.contacts().first();
