...
typedef HRESULT (WINAPI *__SetCurrentProcessExplicitAppUserModelID)(wchar_t* id);
...
YaOnline::YaOnline(YaPsiServer* parent)
	...
	, canConnectConnections_(true) // ONLINE-2890
	, useSsl_(true)
	...
	, checkForAliveOnlineProcessTimer_(0)
{
	...
	if (shell) {
		__SetCurrentProcessExplicitAppUserModelID func;
		func = (__SetCurrentProcessExplicitAppUserModelID)GetProcAddress(shell,
		                             "SetCurrentProcessExplicitAppUserModelID");
		if (func) {
			func(L"ru.yandex.online");
		}
		FreeLibrary (shell);
	}
	...
	connect(parent, SIGNAL(doShowRosterWithoutActivation()), SLOT(doShowRosterWithoutActivation()), Qt::QueuedConnection);
	connect(parent, SIGNAL(doShowRosterMinimized()), SLOT(doShowRosterMinimized()), Qt::QueuedConnection);
	...
	connect(parent, SIGNAL(doSignedFCdataReady(const QString&)), SLOT(doSignedFCdataReady(const QString&)), Qt::QueuedConnection);
	connect(parent, SIGNAL(doSetOverlayIcon(const QString&)), SLOT(doSetOverlayIcon(const QString&)), Qt::QueuedConnection);
...
void YaOnline::doToasterClicked(const QString& id, bool activate)
{
	...
	if (!eventData.isNull()) {
		if (GlobalEventQueue::instance()->ids().contains(eventData.id)) {
			...
			else if (event->type() == PsiEvent::GroupchatAlert || event->type() == PsiEvent::GroupchatInvite) {
				controller_->processEvent(event, activate ? UserAction : UserPassiveAction);
				return;
			}
...
{
	return account->addPendingMessagesForJid(jid);
}
...
void YaOnline::doToasterIgnored(const QString& id)
{
	...
	if (!eventData.isNull()) {
		if (GlobalEventQueue::instance()->ids().contains(eventData.id)) {
			...
			else if (event->type() == PsiEvent::GroupchatAlert) {
				// FIXME: remove event?
			}
			else if (event->type() == PsiEvent::GroupchatInvite) {
				// FIXME: remove event?
			}
			else {
				Q_ASSERT(false);
...
void YaOnline::doShowIgnoredToasters()
{
	...
	foreach(int id, GlobalEventQueue::instance()->ids()) {
		...
		QPointer<PsiEvent> event = GlobalEventQueue::instance()->peek(id);
		Q_ASSERT(event);
		if (!event)
			continue;
		...
		if (event->type() == PsiEvent::Message) {
			if (event->account()) {
				PsiAccount* account = event->account();
				XMPP::Jid from = event->from();

				// we need to open all the chats without activating them in the process
				// event could get deleted as result of this call
				account->actionOpenSavedChat(from.full());
				doToasterIgnored(account, from);
			}
		}
		else if (event->type() == PsiEvent::Auth) {
...
static QVariantMap toasterParams(const QString& type, PsiAccount* account, PsiEvent* event, const XMPP::Jid& jid, const QString& _message, const QDateTime& timeStamp, const QString& callbackId, int soundType)
{
	QString message = Ya::limitText(_message, 300);
	message = TextUtil::plain2richSimple(message);
	bool processMessageNotifierText = true;

	QString contactName = Ya::VisualUtil::contactName(account, jid);
	QString avatarPath = Ya::VisualUtil::scaledAvatarPath(account, jid);
	...
	if (type == "chat") {
		...
		if (messageEvent->multiContactCount() > 1) {
			contactName = Ya::multipleContactName(messageEvent->multiContactCount());
			avatarPath = Ya::VisualUtil::scaledAvatarPath(
			                 Ya::VisualUtil::noAvatarPixmapFileName(XMPP::VCard::MultipleGender),
			                 account);
		}
	...
	else if (type == "gc-alert") {
		showToaster = PsiOptions::instance()->getOption("options.ya.popups.message.enable").toBool();
	}
	else if (type == "gc-invite") {
		showToaster = true;
	}
...
void YaOnline::showToaster(const QString& type, PsiAccount* account, const XMPP::Jid& jid, PsiEvent* event, const QString& message, const QDateTime& timeStamp, const QString& callbackId, int soundType)
{
	QVariantMap map = toasterParams(type, account, event, jid, message, timeStamp, callbackId, soundType);
...
void YaOnline::notify(int id, PsiEvent* event, int soundType)
{
	...
	if (event->type() == PsiEvent::Mood) {
		...
		showToaster("mood",
		            event->account(), event->from(), event,
	...
	if (event->type() == PsiEvent::GroupchatAlert) {
		GroupchatAlertEvent* e = static_cast<GroupchatAlertEvent*>(event);

		showToaster("gc-alert",
		            event->account(), event->from(), event,
		            e->message(),
		            event->timeStamp(),
		            yaOnlineNotifyMid(id, event),
		...
		GroupchatInviteEvent* e = static_cast<GroupchatInviteEvent*>(event);

		showToaster("gc-invite",
		            event->account(), event->from(), event,
		            e->groupchat().full(),
		            event->timeStamp(),
		            yaOnlineNotifyMid(id, event),
...
void YaOnline::closeNotify(int id, PsiEvent* event)
{
	...
	if (event->type() == PsiEvent::Auth ||
	    ...
	    event->type() == PsiEvent::GroupchatAlert ||
	    event->type() == PsiEvent::GroupchatInvite)
	{
...
void YaOnline::updateOnlineAccount()
{
	...
	XMPP::Jid jid(ycuApi_->getUsername());
...
void YaOnline::disconnectRereadSettingsAndReconnect(bool reconnect)
{
	...
	rereadSettings();
	if (reconnect) {
		PsiLogger::instance()->log(QString("YaOnline()::disconnectRereadSettingsAndReconnect(%1)")
		                           .arg(lastStatus_));
...
void YaOnline::doSetOfflineMode()
{
	...
	PsiLogger::instance()->log(QString("YaOnline::doSetOfflineMode()"));
...
void YaOnline::setHaveConnectingAccounts(const QStringList& accounts)
{
	server_->dynamicCall("setHaveConnectingAccounts(const QString&)", accounts.join(";"));
}

void YaOnline::setHaveConnectedAccounts(bool haveConnectedAccounts)
{
	server_->dynamicCall("setHaveConnectedAccounts(const QString&)", haveConnectedAccounts ? "true" : "false");		
}

void YaOnline::networkInterfacesChanged()
{
	server_->dynamicCall("jabberNetInterfaceChanged(const QString&)", QString());
}
...
void YaOnline::eventQueueChanged()
{
...

	{
		...
		skipIds += skipEvents(goodIds, PsiEvent::GroupchatAlert);
		skipIds += skipEvents(goodIds, PsiEvent::GroupchatInvite);
...
void YaOnline::updateActiveProfile()
{
	...
	XMPP::Jid activeProfileJid(activeProfile_);
	if (!activeProfileJid.node().isEmpty() && activeProfileJid.domain() == "ya.ru") {
		activeProfile_ = activeProfileJid.node();
	}
...
static QVariantMap contactParams(const QString& id, PsiAccount* account, const XMPP::Jid& jid, PsiContact* contact)
{
	...
	map["jid"] = Ya::humanReadableJid(account->psi(), jid); // jid.bare();
...
void YaOnline::showConnectionToaster(PsiAccount* account, const QString& error, bool showingError, int errorType)
{
	Q_ASSERT(account);
	if (!account)
		return;
...
{
	showConnectionToaster(account, QString(), false, -1);
}

void YaOnline::showConnectionErrorToaster(PsiAccount* account, const QString& error, int errorType)
{
	showConnectionToaster(account, error, true, errorType);
}
...
void YaOnline::doJSetCanConnectConnections(bool _canConnectConnections)
{
	PsiLogger::instance()->log(QString("YaOnline::doJSetCanConnectConnections(%1)").arg(_canConnectConnections));

	// ONLINE-2890
#if 0
	// if we were connected once, do so in the future
	bool canConnectConnections = canConnectConnections_ || _canConnectConnections;
...
void YaOnline::doShowRosterWithoutActivation()
{
	emit showRosterWithoutActivation();
}

void YaOnline::doShowRosterMinimized()
{
	emit showRosterMinimized();
}
...
bool YaOnline::forceMinimizeOnClose() const
{
	enum TShowAppIcon {
		ShowAppIcon_TaskbarOnly = 0,
		ShowAppIcon_TrayOnly = 1,
		ShowAppIcon_TaskbarAndTray = 2
	};

	QSettings sUser(QSettings::UserScope, "Yandex", "Online");
	int value = sUser.value("chat/show_app_icon", "0").toString().toInt();
	if (value < 0 || value > 2)
		value = 0;
	return value == ShowAppIcon_TaskbarOnly ||
	       value == ShowAppIcon_TaskbarAndTray;
}
...
void YaOnline::foundContact(PsiAccount* account, const XMPP::Jid& jid, bool inRoster, bool authorized, bool selfContact)
{
	...
	map["human_jid"] = Ya::humanReadableJid(controller_, jid);
...
void YaOnline::setStatusTextUserAction(const QString& statusText)
{
	server_->dynamicCall("setStatusTextUserAction(const QString&)", statusText);
}

void YaOnline::xFacebookPlatformLogin(const QVariantMap& map, QObject* obj, const char* slot)
{
	xFacebookPlatformLoginCallbackObject_ = obj;
	xFacebookPlatformLoginCallbackSlot_ = slot;

	QString json = CuteJson::variantToJson(map);
	server_->dynamicCall("signFCdata(const QString&)", json);
}

void YaOnline::doSignedFCdataReady(const QString& data)
{
	if (xFacebookPlatformLoginCallbackObject_) {
		QMetaObject::invokeMethod(xFacebookPlatformLoginCallbackObject_, xFacebookPlatformLoginCallbackSlot_,
		                          Qt::DirectConnection, QGenericReturnArgument(),
		                          Q_ARG(QString, data));
	}

	xFacebookPlatformLoginCallbackObject_ = 0;
	xFacebookPlatformLoginCallbackSlot_ = QString();
}

void YaOnline::doSetOverlayIcon(const QString& json)
{
	if (!mainWin_.isNull()) {
		mainWin_->setOverlayIcon(json);
	}
}
