...
YaDayUse* YaDayUse::instance_ = 0;

YaDayUse* YaDayUse::instance(PsiCon* controller)
{
	if (!instance_) {
		instance_ = new YaDayUse(controller);
		instance_->init();
	}
	return instance_;
}
...
void YaDayUse::stat(int pid, int cid)
{
	QString url = QString("http://clck.yandex.ru/redir/dtype=stred/pid=%1/cid=%2/*http://online.yandex.ru")
	              .arg(pid)
	              .arg(cid);
	httpGet(url);
}
...
{
	return "http://soft.export.yandex.ru/status.xml";
}
...
{
	return QString("%1.%2").arg(YAPSI_VERSION).arg(YAPSI_REVISION);
}
...
YaDayUse::YaDayUse(PsiCon* controller)
	: BaseHttpHelper<QObject>(controller, controller)
{
	QTimer::singleShot(0, this, SLOT(dayUse()));
}
...
#else
	QString url = QString("%1?yasoft=%2&ui=%3&ver=%4&os=%5&osver=%6&stat=dayuse")
	              .arg(basePingUrl())
	              .arg(appId())
	              .arg(ui())
	              .arg(ver())
...
{
	QString url = _url;
	network()->get(HttpHelper::getRequest(url));
}
...
void YaDayUse::replyFinished(QNetworkReply* reply)
{
	reply->deleteLater();
}
