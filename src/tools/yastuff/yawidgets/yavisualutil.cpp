...
private:
		...
		: QObject(QCoreApplication::instance())
	{
		...
		foreach(QString bitmapFileName, bitmaps) {
			int result = rx.indexIn(bitmapFileName);
			Q_ASSERT(result >= 0);
...
QString VisualUtil::scaledAvatarPath(PsiAccount* account, const XMPP::Jid& jid, int toasterAvatarSize)
{
	...
	return scaledAvatarPath(fileName, account, toasterAvatarSize);
}

QString Ya::VisualUtil::scaledAvatarPath(const QString& fileName, PsiAccount* account, int toasterAvatarSize)
{
...
QPixmap VisualUtil::rosterStatusPixmap(XMPP::Status::Type status)
{
	...
	case XMPP::Status::NotAuthorizedToSeeStatus:
		...
		break;
	case XMPP::Status::GC_Active:
		result = QPixmap(":iconsets/roster/default/gc-available.png");
		break;
	case XMPP::Status::GC_Inactive:
		result = QPixmap(":iconsets/roster/default/gc-unavailable.png");
		break;
	case XMPP::Status::GC_Favorited:
		result = QPixmap(":iconsets/roster/default/gc-fav.png");
		break;
#endif
	default:
		Q_ASSERT(false);
...
QIcon Ya::VisualUtil::groupchatAvatar(XMPP::Status::Type active)
{
	Q_UNUSED(active);
	return QIcon(":images/groupchat/gc-available.png");
}
...
QString Ya::VisualUtil::noAvatarPixmapFileName(XMPP::VCard::Gender gender)
{
	...
	case XMPP::VCard::MultipleGender:
		return ":/images/gender/default_multiple_avatar.png";
	}
	Q_ASSERT(false);
	return QString();
}
...
const QPixmap& VisualUtil::noAvatarPixmap(XMPP::VCard::Gender gender)
{
	...
	case XMPP::VCard::Female:
		...
		return pix_female;
	case XMPP::VCard::MultipleGender:
		if (pix_multiple.isNull())
			pix_multiple = QPixmap(noAvatarPixmapFileName(gender));
		return pix_multiple;
	}
	Q_ASSERT(false);
...
int Ya::VisualUtil::windowShadowSize(const YaWindowTheme& theme)
{
	return theme.mode() != YaWindowTheme::Unthemable ?
	       9 : 17;
}
...
void Ya::VisualUtil::drawWindowTheme(QPainter* painter, const YaWindowTheme& theme, const QRect& rect, const QRect& contentsRect, bool showAsActiveWindow)
{
	...
	if (theme.customFrame()) {
		...
		BorderRenderer* border = BorderRenderer::rendererFor(
		                             theme.mode() != YaWindowTheme::Unthemable ?
		                             ":images/window/shadow" :
		                             ":images/window/shadow2");
		border->draw(painter, shadowRect.adjusted(0, 0, 1, 1));

		QRegion mask = Ya::VisualUtil::roundedMask(contentsRect.size(), windowCornerRadius(theme), Ya::VisualUtil::TopBorders);
