...
{
	return Ya::VisualUtil::windowCornerRadius(theme());
}
...
#ifdef USE_YAVISUALUTIL
	Ya::VisualUtil::Borders borders = theme().mode() != YaWindowTheme::Unthemable ?
	                                  Ya::VisualUtil::TopBorders : Ya::VisualUtil::AllBorders;
	return Ya::VisualUtil::roundedMask(size(), cornerRadius(), !square ? borders : Ya::VisualUtil::NoBorders);
#else
...
void YaWindowBase::paintEvent(QPaintEvent*)
{
	...
	paint(&p);
	if (theme().mode() != YaWindowTheme::Unthemable) {
...
{
#ifdef CUSTOM_SHADOW
	return Ya::VisualUtil::windowShadowSize(theme());
#else
	return 0;
#endif
}
