...
YaChatSeparatorPushButton::YaChatSeparatorPushButton(const QString& text, bool doWrap, QWidget* parent)
...
{
	QSize result = QSize(fontMetrics_.width(text()) + 10 * 2, 16);
	if (doWrap_) {
		result.setWidth(result.width() - 30);
	}
	return result;
}
...
void YaChatSeparatorPushButton::drawForeground(QPainter* p)
{
	...
	p->drawText(textRect, Qt::AlignCenter | Qt::AlignVCenter | Qt::TextWordWrap, text());
}
...
//----------------------------------------------------------------------------
// YaChatSeparatorUploadButton
//----------------------------------------------------------------------------

YaChatSeparatorUploadButton::YaChatSeparatorUploadButton(QWidget* parent)
	: YaChatSeparatorPushButton(QString(), false, parent)
{
	setToolTip(tr("Send File"));
	PsiToolTip::install(this);

	icon_ = QPixmap(":images/chat/upload.png");
	iconHover_ = QPixmap(":images/chat/upload.png");
}

QSize YaChatSeparatorUploadButton::sizeHint() const
{
	QSize sh = YaChatSeparatorPushButton::sizeHint();
	sh.setWidth(icon_.width());
	return sh;
}

void YaChatSeparatorUploadButton::drawForeground(QPainter* p)
{
	QPixmap pix = isDown() || underMouse() ? iconHover_ : icon_;
...
YaChatEmoticonToolBar::YaChatEmoticonToolBar(QWidget* parent)
	: QToolBar(parent)
{
	style_ = new YaChatEmoticonToolBarStyle(qApp->style());
	setStyle(style_);
...
YaChatEmoticonToolBar::~YaChatEmoticonToolBar()
{
	delete style_;
}
	...
	, overlayParent_(overlayParent)
{
	...
	addButton_ = new YaChatSeparatorPushButton(tr("Add"), false, this);
	connect(addButton_, SIGNAL(clicked()), SIGNAL(addContact()));
	addButton_->hide();
	authButton_ = new YaChatSeparatorPushButton(tr("Auth"), false, this);
	connect(authButton_, SIGNAL(clicked()), SIGNAL(authContact()));
	authButton_->hide();
	sendButton_ = new YaChatSeparatorUploadButton(this);
	connect(sendButton_, SIGNAL(clicked()), SIGNAL(sendFile()));
	// sendButton_->hide();
...
int YaChatSeparatorExtra::normalWidth() const
{
	...
	if (sendButton_->isVisible())
		w += sendButton_->sizeHint().width();
...
void YaChatSeparatorExtra::updateChildWidgets()
{
	...
	if (sendButton_->isVisible()) {
		QRect sendButtonRect(rect());
		sendButtonRect.setRight(sendButtonRect.right() - widthLimit);
		sendButtonRect.setLeft(sendButtonRect.right() - sendButton_->sizeHint().width());
		sendButton_->setGeometry(sendButtonRect);

		widthLimit += sendButtonRect.width();
	}
	...
	, resizingChatEdit_(false)
{
	...
	connect(extra(), SIGNAL(sendFile()), SIGNAL(sendFile()));
...
YaChatSeparator::~YaChatSeparator()
{
}
...
void YaChatSeparator::optionChanged(const QString& option)
{
	if (option == emoticonsPanelEnabledOptionPath) {
		...
		const int numLines = this->numLines();
...
int YaChatSeparator::numLines() const
{
	return qMax(1, qMin(100, PsiOptions::instance()->getOption(editorHeightOptionPath).toInt()));
}

// based on LineEdit::minimumSizeHint() code
int YaChatSeparator::chatEditHeight(const int numLines) const
{
...
int YaChatSeparator::chatEditMaximumHeight() const
{
	if (!window()->isVisible()) {
		return chatEditHeight(numLines());
	}
