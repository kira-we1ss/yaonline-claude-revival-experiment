...
void YaManageAccounts::setController(PsiCon* controller)
{
	...
	connect(controller->yaTransportManager(), SIGNAL(availableTransportsChanged()), modelChangedTimer_, SLOT(start()));
...
bool YaManageAccounts::eventFilter(QObject* obj, QEvent* e)
{
	if (e->type() == QEvent::ContextMenu && obj == ui_.treeView) {
		...
		if (!contextMenuEvent->modifiers().testFlag(Qt::AltModifier) &&
		    !contextMenuEvent->modifiers().testFlag(Qt::ControlModifier))
			return true;
...
void YaManageAccounts::deleteAccount(const QString& xml)
{
	...
	YaTransport* transport = controller_->yaTransportManager()->findTransport(e.attribute("id"));
	PsiAccount* account = controller_->contactList()->getAccount(e.attribute("id"));

	if (account || transport) {
		QString accountName = account ? account->name() : transport->jid();
		QRegExp facebookRx("^\\-\\d+\\@chat\\.facebook\\.com$");
		if (facebookRx.exactMatch(accountName)) {
...
{
	QDomDocument doc;
	if (!controller_ || !controller_->contactList() || !controller_->yaTransportManager())
		return doc;
	...
	foreach(const YaTransport* transport, controller_->yaTransportManager()->transports()) {
		if (!transport->isAvailable() && transport->errorMessage().isEmpty())
			continue;

		QDomElement tag = textTag(&doc, "account", QString());
		tag.setAttribute("id", transport->id());
		tag.setAttribute("avatar", transport->avatarPath(50));
		tag.setAttribute("status", Ya::statusToFlash(transport->status()));
		tag.setAttribute("builtin", (int)false);
		tag.setAttribute("error_message", transport->errorMessage());
		QString jid = transport->transportLogin().isEmpty() ?
		              transport->jid() :
		              transport->transportLogin();
		tag.setAttribute("jid", jid);
		tag.setAttribute("password", QString());

		tag.setAttribute("jabber_service", "transport");
		tag.setAttribute("ssl", false);
		tag.setAttribute("server_history", false);

		tag.setAttribute("use_manual_host", "0");
		tag.setAttribute("manual_host", QString());
		tag.setAttribute("manual_port", 0);
		root.appendChild(tag);
	}
...
{
	PsiLogger::instance()->log(QString("applyImmediatePreferencesHelper: (%1, %2)").arg(xml).arg(confirmation));
	if (!controller_)
		return;
	...
	if (root.attribute("type") == "mrim_settings") {
		YaTransport* transport = controller_->yaTransportManager()->findTransport("MRIM");
		if (transport) {
			transport->doSettings();
		}
		return;
	}

	if (!root.attribute("type").startsWith("account_"))
		return;
	...
	if (root.attribute("confirmation") == "true")
		confirmation = true;
	...
	for (QDomNode n = root.firstChild(); !n.isNull(); n = n.nextSibling()) {
		...
		if (e.tagName() == "account") {
			YaTransport* transport = controller_->yaTransportManager()->findTransport(e.attribute("id"));
			if (transport) {
				if (actionType == Account_Connect || actionType == Account_Disconnect) {
					transport->setConnected(actionType == Account_Connect);
				}
				else if (actionType == Account_Remove) {
					transport->remove();
				}
					...
					if (e.hasAttribute("server_history")) {
						...
						updateServerHistory(transport->account(), transport->jid(), enable);
						continue;
					}

					transport->registerTransport(e.attribute("jid"), e.attribute("password"));
				}
				continue;
			}
...
bool YaManageAccounts::updateUserAccount(PsiAccount* account, const QDomElement& e, bool forceReconnect) const
{
	...
	if (e.hasAttribute("server_history")) {
		...
		updateServerHistory(account, XMPP::Jid(), enable);
	}
	...
	if (forceReconnect || (saveable && reconnect && account->enabled())) {
		account->setEnabled(false);
		account->setEnabled(!ua.pass.isEmpty() || (ua.id == "FACEBOOK"));
		account->autoLogin();
	}
...
void YaManageAccounts::updateServerHistory(PsiAccount* account, const XMPP::Jid& transportJid, bool enable) const
{
	...
	if (!transportJid.isNull()) {
		m.setTwin(transportJid.bare());
	}

	if (baseAccount && (baseAccount->isYaAccount() || baseAccount->isYaPddAccount())) {
