...
static QColor& nickColor(int id, bool self)
{
	static QColor selfColor("#2222ff");
	if (self) {
		return selfColor;
	}
	...
	if (colors.isEmpty()) {
		// colors << QColor("#2222ff");
		colors << QColor("#ff0006");
		colors << QColor("#4ccb00");
		colors << QColor("#c6c800");
		colors << QColor("#1191e2");
		colors << QColor("#ffc600");
		colors << QColor("#c600ff");
		colors << QColor("#ff0078");
		colors << QColor("#00d3f9");
		colors << QColor("#f98100");
		colors << QColor("#00e19f");
	}
	return colors[(colors.count()-1) % id];
}

static QPixmap deliveryConfirmationPixmap(YaChatViewModel::DeliveryConfirmationType type)
{
	static QPixmap pix_ok;
	static QPixmap pix_server_ok;
	static QPixmap pix_server_fail;
	static QPixmap pix_fail;

	switch (type) {
	case YaChatViewModel::DeliveryConfirmation_Verified:
		if (pix_ok.isNull()) {
			pix_ok = QPixmap(":images/chat/delivery_confirmation_ok.png");
		}
		return pix_ok;
	case YaChatViewModel::DeliveryConfirmation_DeliveredToLocalServer:
		if (pix_server_ok.isNull()) {
			pix_server_ok = QPixmap(":images/chat/delivery_confirmation_server_ok.png");
		}
		return pix_server_ok;
	case YaChatViewModel::DeliveryConfirmation_ServerPingTimeout:
		if (pix_server_fail.isNull()) {
			pix_server_fail = QPixmap(":images/chat/delivery_confirmation_server_fail.png");
		}
		return pix_server_fail;
...
void YaChatViewDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
	...
	case YaChatViewModel::DummyHeader:
		break;
	case YaChatViewModel::GC_Separator:
		painter->fillRect(option.rect, Qt::black);
		break;
	case YaChatViewModel::Message:
	default:
		Q_ASSERT(type(index) == YaChatViewModel::Message ||
		         type(index) == YaChatViewModel::MoodChanged ||
		         type(index) == YaChatViewModel::StatusTypeChangedNotice ||
		         type(index) == YaChatViewModel::FileUpload ||
		         index.data(YaChatViewModel::PersistentNoticeRole).toBool());
...
bool YaChatViewDelegate::isHoveringDeliveryConfirmation(const QModelIndex& index, const QPoint& pos) const
{
	...
	switch (type(index)) {
	case YaChatViewModel::FileUpload:
	case YaChatViewModel::Message: {
		ensureMessageLayout(QStyleOptionViewItem(), index);
		DelegateMessageData data = ensureDelegateMessageData(index);
...
void YaChatViewDelegate::sendControlEvent(const QModelIndex& index, QMouseEvent* e, const QPointF& coordinateOffset, QWidget* contextWidget)
{
	...
	case YaChatViewModel::GC_Separator:
		break;
	case YaChatViewModel::FileUpload: {
		QAbstractScrollArea* scrollArea = dynamic_cast<QAbstractScrollArea*>(contextWidget);
		QRect baseRect = scrollArea->rect();
		if (scrollArea->verticalScrollBar()->isVisible()) {
			baseRect.setRight(baseRect.right() - scrollArea->verticalScrollBar()->width());
		}

		QRect r(baseRect);
		r.setTopLeft(r.topLeft() + messageTextControlPosition(index));

		QList<QPixmap> closePixmaps = Ya::VisualUtil::closeTabButtonPixmaps();
		QPixmap closePixmap = closePixmaps.first();

		r.setRight(r.right() - closePixmap.width() - 10);

		QRect closeRect(baseRect);
		closeRect.setTopLeft(r.topRight() + QPoint(5, 0));
		closeRect.setSize(closePixmap.size());

		QPoint adjustedPos = e->pos() + coordinateOffset.toPoint();
		// qWarning() << closeRect << e->pos() << adjustedPos;
		if (closeRect.contains(adjustedPos)) {
			if (e->type() == QEvent::MouseButtonPress) {
				YaChatViewModel* model = dynamic_cast<YaChatViewModel*>(const_cast<QAbstractItemModel*>(index.model()));
				Q_ASSERT(model);
				model->removeFileUpload(index.data(YaChatViewModel::FU_Id).toString());
			}
		}
		break;
	}
	...
	if (width() != data.sizeHint.width()) {
		...
		if (!data.textControl) {
			...
			if (type(index) == YaChatViewModel::ContactComposing ||
			    ...
			    type(index) == YaChatViewModel::StatusTypeChangedNotice)
			{
				messageText = messageSenderName(index) + messageText;
			}

			if (mode_ == Mode_Groupchat) {
				messageText = messageSenderNameAsRichText(index) + "&nbsp;&nbsp;" + messageText;
			}
...
QString YaChatViewDelegate::messageSenderName(const QModelIndex& index) const
{
	if (mode_ == Mode_Groupchat) {
		return index.data(YaChatViewModel::GC_NickRole).toString();
	}

	bool incoming = index.data(YaChatViewModel::IncomingRole).toBool();
	if (incoming)
		return contactNick();
...
QString YaChatViewDelegate::messageSenderNameAsRichText(const QModelIndex& index) const
{
	QString result = messageSenderName(index);
	if (result.length() > 0) {
		QString tmp = result;
...
void YaChatViewDelegate::drawMessageName(QPainter* painter, QRect& nameRect, const QModelIndex& index) const
{
	bool incoming = index.data(YaChatViewModel::IncomingRole).toBool();
	QString name = TextUtil::unescape(messageSenderName(index));
...
void YaChatViewDelegate::drawMessage(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
	...
	if (deliveryConfirmation == YaChatViewModel::DeliveryConfirmation_Error
	    || deliveryConfirmation == YaChatViewModel::DeliveryConfirmation_Timeout
	    /*|| deliveryConfirmation == YaChatViewModel::DeliveryConfirmation_ServerPingTimeout*/)
	{
		painter->fillRect(option.rect, QColor(0xFD,0xCE,0xD0));
	}
	else if (type(index) == YaChatViewModel::ContactComposing) {
	...
	if (deliveryConfirmation != YaChatViewModel::DeliveryConfirmation_Unknown) {
		...
		case YaChatViewModel::DeliveryConfirmation_Querying:
			break;
		case YaChatViewModel::DeliveryConfirmation_Verified:
			pix = deliveryConfirmationPixmap(YaChatViewModel::DeliveryConfirmation_Verified);
			break;
		case YaChatViewModel::DeliveryConfirmation_DeliveredToLocalServer:
			pix = deliveryConfirmationPixmap(YaChatViewModel::DeliveryConfirmation_DeliveredToLocalServer);
			break;
		case YaChatViewModel::DeliveryConfirmation_Error:
		case YaChatViewModel::DeliveryConfirmation_Timeout:
			pix = deliveryConfirmationPixmap(YaChatViewModel::DeliveryConfirmation_Error);
			break;
		case YaChatViewModel::DeliveryConfirmation_ServerPingTimeout:
			pix = deliveryConfirmationPixmap(YaChatViewModel::DeliveryConfirmation_ServerPingTimeout);
			break;
		default:
			;
		}
	...
	if (!index.data(YaChatViewModel::MergeRole).toBool() && mode_ == Mode_Chat) {
		...
		drawMessageName(painter, nameRect, index);

		y += nameFontMetrics_->height() + namePadding;
	}
	else if (mode_ == Mode_Groupchat) {
		painter->setPen(lineColor_);
		// QPoint lineOffset(lineMargin, 0);
		// painter->drawLine(option.rect.topLeft() + lineOffset, option.rect.topRight() - lineOffset);
		y += mergedTopMargin;

		if (index.data(YaChatViewModel::GC_ShowDateTime).toBool()) {
			QRect timeRect(option.rect);
			QString time = index.data(YaChatViewModel::DateTimeRole).toDateTime().toString("hh:mm");
			timeRect.setTop(timeRect.top() + y + timePadding);
			timeRect.setLeft(timeRect.right() - rightMargin - timeFontMetrics_->width(time));
	...
	if (type(index) == YaChatViewModel::FileUpload) {
		QRect r(option.rect);
		r.setTopLeft(option.rect.topLeft() + messageTextControlPosition(index));

		int stateTmp = index.data(YaChatViewModel::FU_State).toInt();
		YaNarodDiskManager::State state = static_cast<YaNarodDiskManager::State>(stateTmp);
		if (state != YaNarodDiskManager::State_Error) {
			QList<QPixmap> closePixmaps = Ya::VisualUtil::closeTabButtonPixmaps();
			QPixmap closePixmap = closePixmaps.first();
			...
			QRect closeRect(option.rect);
			closeRect.setTopLeft(r.topRight() + QPoint(5, 0));
			closeRect.setSize(closePixmap.size());
			painter->drawPixmap(closeRect, closePixmap);
		}

		painter->setFont(noticeFont_);
		painter->setPen(textColor_);

		QString msg = tr("Sending file");
		double percent = (index.data(YaChatViewModel::FU_BytesSent).toDouble() / index.data(YaChatViewModel::FU_BytesTotal).toDouble()) * 100.0;
		QString text;
		if (!index.data(YaChatViewModel::FU_BytesTotal).toULongLong()) {
			text = trUtf8("%1: %2")
			       .arg(msg)
			       .arg(YaNarodDiskManager::humanReadableName(
			                index.data(YaChatViewModel::FU_FileName).toString(),
			                index.data(YaChatViewModel::FU_BytesTotal).toULongLong()));
		}
		else {
			text = trUtf8("%1: %2 — %3%")
			       .arg(msg)
		...
		painter->drawText(r, Qt::AlignLeft | Qt::AlignVCenter | Qt::TextSingleLine,
		                  noticeFontMetrics_->elidedText(text, Qt::ElideRight, r.width()));
	}
	else {
...
QString YaChatViewDelegate::selectedMessageText(const QModelIndex& index, SelectedText selectedText) const
{
	...
	if (selectedText == SelectedTextWithHeader) {
		...
		result = formatStr.arg(timestr)
				  .arg(TextUtil::unescape(messageSenderName(index)))
		                  .arg(result);
	}
...
QString YaChatViewDelegate::selectedMessageHtml(const QModelIndex& index, SelectedText selectedText) const
{
	...
	if (selectedText == SelectedTextWithHeader) {
		...
		QString name = messageSenderName(index);
		if (mode_ == Mode_Groupchat) {
			name = QString();
		}
...
YaChatViewDelegate::Mode YaChatViewDelegate::mode() const
{
	return mode_;
}

void YaChatViewDelegate::setMode(YaChatViewDelegate::Mode mode)
{
	mode_ = mode;
}
