	...
	, statusTextEscape_(false)
{
	...
	vCardAction_ = new QAction(tr("vCard"), this);
	...
	jidMenu_->addAction(vCardAction_);
}
...
void YaRosterTipLabel::invalidateData()
{
	...
	ui_.renameButton->setEnabled(contact_->isRenameable());
	...
	ui_.jidLabel->setText(Ya::humanReadableJid(contact_->account()->psi(), contact_->jid()));
...
{
	closeClicked();
	emit addContact(Ya::humanReadableJid(contact_->account()->psi(), contact_->jid()),
	                contact_->account(), QStringList(), QString());
}
...
void YaRosterTipLabel::setStatusText(QString text, bool invisible, bool escape)
{
	...
	if (escape) {
		...
		tmp = TextUtil::linkify(TextUtil::plain2rich(tmp));
	}
...
void YaRosterTipLabel::copyJid()
{
	...
	if (!contact_)
		return;
	QApplication::clipboard()->setText(Ya::humanReadableJid(contact_->account()->psi(), contact_->jid()));
}
