...
namespace Ya {
	...
	QStringList filterGroupNames(QStringList groups);
	...
	QString multipleContactName(int count);
	...
	QString stripYaRuDomain(const QString& jid);
	bool isYaRuDomain(const QString& domain);
	PsiContact* findContact(PsiCon* controller, const QString& jid);
	QString humanReadableJid(PsiCon* controller, const XMPP::Jid& jid);
	QString contactListFilterRegExp(const QString& text);
	void yandexSearch(const QString& string);
	...
	void showHistory(const PsiAccount* me, const XMPP::Jid& interlocutor);
