...
void GAdvancedWidget::Private::saveGeometry()
{
	...
	PsiOptions::instance()->setOption(visibilityOptionPath_, parentWidget_->isVisible());
#endif
}
...
void GAdvancedWidget::Private::restoreGeometry()
{
	...
	bool shouldBeVisible = PsiOptions::instance()->getOption(visibilityOptionPath_).toBool();
	...
	if (parentWidget_->property("disable-show-on-restore").toBool()) {
		return;
	}
	...
	if (!shouldBeVisible) {
		...
		gAdvancedWidget_->showWithoutActivation();
	}
#endif
}
...
{
	QRect geom = gAdvancedWidget_->ensureGeometryVisible(savedGeometry);
	parentWidget_->setGeometry(geom);
}
...
{
	if (msg->message == WM_WINDOWPOSCHANGING && GAdvancedWidget::Private::stickEnabled) {
...
void GAdvancedWidget::setGeometryOptionPath(const QString& optionPath)
{
	...
	d->visibilityOptionPath_ = optionPath + "-visible";
	d->restoreGeometry();
	d->parentWidget_->installEventFilter(d);
}

void GAdvancedWidget::restoreSavedGeometry()
{
	d->restoreGeometry();
}
...
QRect GAdvancedWidget::ensureGeometryVisible(QRect _geom)
{
	QRect geom = _geom;
#ifdef YAPSI
	YaWindow* yaWindow = dynamic_cast<YaWindow*>(d->parentWidget_);
	if (yaWindow)
		geom = yaWindow->frameGeometryToContentsGeometry(geom);
#endif
	...
	if (geom.width() > r.width())
		geom.setWidth(r.width());
	if (geom.height() > r.height())
		geom.setHeight(r.height());
...
#ifdef YAPSI
	if (yaWindow)
		geom = yaWindow->contentsGeometryToFrameGeometry(geom);
#endif
	return geom;
}
