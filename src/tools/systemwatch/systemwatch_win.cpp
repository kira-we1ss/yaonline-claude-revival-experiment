...
bool WinSystemWatch::processWinEvent(MSG *m, long* /* result */)
{
	...
	if(WM_POWERBROADCAST == m->message) {
		PsiLogger::instance()->log(QString("%1 WinSystemWatch::processWinEvent() WM_POWERBROADCAST %2").arg(LOG_THIS).arg(m->wParam));
		...
		PsiLogger::instance()->log(QString("%1 WinSystemWatch::processWinEvent() WM_QUERYENDSESSION %2").arg(LOG_THIS).arg(m->wParam));
