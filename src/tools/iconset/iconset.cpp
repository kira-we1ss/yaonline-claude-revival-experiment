...
public:
	...
	: QObject(), Q3Shared()
	{
		...
		sound = from.sound;
#ifdef YAPSI
		isUserSelectable = from.isUserSelectable;
...
#ifdef YAPSI
/**
 * Returns \a true if this icon shouldn't appear in emoticon selector.
 */
bool PsiIcon::isUserSelectable() const
{
	return d->isUserSelectable;
}

/**
 * Set whether the icon should appear in emoticon selector.
 */
void PsiIcon::setUserSelectable(bool userSelectable)
{
	d->isUserSelectable = userSelectable;
}

/**
 * File name from which this icon was loaded.
 */
QString PsiIcon::fileName() const
{
	return d->fileName;
}

void PsiIcon::setFileName(const QString& fileName)
{
	d->fileName = fileName;
}

QString PsiIcon::description() const
{
	return d->description;
}

void PsiIcon::setDescription(const QString& description)
{
...
private:
	...
	QList<QPointer<Iconset> > iconsets_;
...
{
	// Q_ASSERT(!iconsets_.contains((Iconset*)i));
...
public:
	...
	bool loadIcon(const QDomElement &i, const QString &dir)
	{
...
#ifdef YAPSI
				else if ( attr == "ya:user-selectable" ) {
					isUserSelectable = e.text() == "true";
				}
				else if ( attr == "ya:description" ) {
		...
		icon.setName(name);
#ifdef YAPSI
		icon.setUserSelectable(isUserSelectable);
...
#ifdef YAPSI
						icon.setFileName(this->filename + "/" + graphic[*it]);
#endif
						loadSuccess = true;
						break;
					}
