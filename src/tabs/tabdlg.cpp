	...
	, reordering_(false)
{
	...
	Ya::initializeDefaultGeometry(this, lastGeometryOptionPath, QRect(0, 0, 600, 390), true);
...
XMPP::Status::Type TabDlg::tabStatus(TabbableWidget* tab) const
{
	...
	if (chatDlg) {
		...
		GCMainDlg* groupchatDlg = dynamic_cast<GCMainDlg*>(tab);
		if (groupchatDlg) {
			status = groupchatDlg->isConnected() ?
