...
public:
	...
	DeliveryConfirmationManager* deliveryConfirmationManager() const;
	CapsManager* capsManager() const;
	VoiceCaller* voiceCaller() const;
	Status status() const;
	Status loggedOutStatus() const;
	void setStatusDirect(const XMPP::Status &, bool withPriority = false);
	void setStatusActual(const XMPP::Status &);
	bool loggedIn() const;
	void setNick(const QString &);
	QString nick() const;
	void forceDisconnect(bool fast = false, const XMPP::Status &s = Status("", "Logged out", 0, false));
#ifdef YAPSI
	QString mood() const;
	QDateTime reconnectScheduledAt() const;
#endif
	bool hasPGP() const;
...
public slots:
...
#ifdef YAPSI
	void dj_authInternal(const Jid &, bool doAuthReq = true);
#endif
	...
	void actionJoin(const Jid& mucJid, const QString& password = QString(), bool connectImmediately = false);
...
private slots:
	...
	void groupchatEventCreated(PsiEvent*);
...
#ifdef YAPSI
	void moodChanged(const QString& resource, const QString& mood);

	void checkLoginPrerequisites();
...
public:
	bool getExpandedState(QString name) const;
	void setExpandedState(QString name, bool expanded);
	...
	bool isYaPddAccount() const;
	bool isOnlineAccount() const;
	bool connectionEnabled() const;
	bool disableAutoConnect() const;

	void resetLastManualStatusSafeGuard();
	bool addPendingMessagesForJid(const XMPP::Jid& jid);
#endif
...
private:
...
#ifdef YAPSI
	void logEventOnServer(const Jid &j, PsiEvent *e, const PsiAccount* account);
#endif
	...
	friend class PsiCon;
#ifdef YAPSI
	friend class YaUnreadMessagesManager;
#endif
