...
private slots:
	...
	void dumpXml(const QDateTime& dateTime, const QString& xml, bool incoming);
...
class XmlPrompt : public QDialog
{
	Q_OBJECT
public:
	XmlPrompt(QWidget *parent=0, const char *name=0);
	~XmlPrompt();
...
private slots:
	void doTransmit();
	void transmitMalformedXmlConfirmation(const QString& id, bool confirmed);
	void transmitHelper(const QString& xml);
