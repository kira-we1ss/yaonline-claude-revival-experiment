...
MUCManager::MUCManager(Client* client, const Jid& room, QObject* parent)
	: QObject(parent)
	, client_(client)
	, room_(room)
{
...
{
	XMPP::XData data;
	if (!roomTitle_.isEmpty()) {
		data.addField(XMPP::XData::Field::Field_TextSingle, "muc#roomconfig_roomname", roomTitle_);
	}
	if (!roomPassword_.isEmpty()) {
		data.addField(XMPP::XData::Field::Field_Boolean,     "muc#roomconfig_passwordprotectedroom", QString::number(1));
		data.addField(XMPP::XData::Field::Field_TextPrivate, "muc#roomconfig_roomsecret", roomPassword_);
	}
...
QString MUCManager::roomTitle() const
{
	return roomTitle_;
}

void MUCManager::setRoomTitle(const QString& roomTitle)
{
	roomTitle_ = roomTitle;
}

void MUCManager::setRoomPassword(const QString& password)
{
