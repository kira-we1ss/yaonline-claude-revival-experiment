...
public slots:
	...
	{
		if (quitting || !restoredSavedChats)
			return;
	...
	{
		PsiLogger::instance()->log(QString("PsiCon::Private::networkInterfacesChanged (sleeping = %1)").arg(psi->computerSleeping()));
		if (!contactList || !contactList->accountsLoaded() || psi->computerSleeping()) {
			return;
		}
...
#ifdef YAPSI_ACTIVEX_SERVER
			bool saveable = account->userAccount().saveable;
			if (!saveable)
				continue;
#endif
...
#ifdef YAPSI_ACTIVEX_SERVER
		psi->yaOnline()->networkInterfacesChanged();
#endif
	}
...
public slots:
	...
	{
		if (restoredSavedChats)
			return;
		restoredSavedChats = true;
		...
		if (currentContact) {
			foreach(TabDlg* tabDlg, tabManager->tabSets()) {
				tabDlg->setProperty("disable-geometry-saving", true);
				...
				tabDlg->setProperty("disable-geometry-saving", false);
				tabDlg->restoreSavedGeometry();
...
public:
	...
	bool restoredSavedChats;
	bool quitting;
	QTimer* updatedAccountTimer_;
	NetworkInterfaceManager *networkInterfaceManager_;
};
...
bool PsiCon::init(bool autoStart)
{
	LOG_TRACE;
	...
	d->networkInterfaceManager_ = new NetworkInterfaceManager(this);
	connect(d->networkInterfaceManager_, SIGNAL(networkInterfacesChanged()), d, SLOT(networkInterfacesChanged()));
...
#ifdef YAPSI
	d->mainwin = new YaMainWin(option.alwaysOnTop, (option.useDock && option.dockToolMW), this, "yapsimain"); 
	d->mainwin->init();
#else
...
// #endif
...
#ifdef YAPSI_ACTIVEX_SERVER
		d->mainwin->setProperty("disable-show-on-restore", true);
#endif
	...
	// ONLINE-3587: try to ensure that global shortcuts work fine on Linux
	setShortcuts();
	...
	yaLogEventsManager_ = new YaLogEventsManager(this);
	yaUnreadMessagesManager_ = new YaUnreadMessagesManager(this);
	yaNarodDiskManager_ = new YaNarodDiskManager(this);
	yaTransportManager_ = new YaTransportManager(this);
	yaMucManager_ = new YaMucManager(this);
#endif
...
void PsiCon::deinit()
{
	...
	delete yaHistoryCacheManager_;
	yaHistoryCacheManager_ = 0;

	delete yaLogEventsManager_;
	yaLogEventsManager_ = 0;
	...
	delete yaTransportManager_;
	yaTransportManager_ = 0;

	delete yaMucManager_;
	yaMucManager_ = 0;
#endif
...
void PsiCon::doGroupChat()
{
	...
	BaseMucJoinDlg* w = (BaseMucJoinDlg*)dialogFind("YaMucJoinDlg");
	if (!w) {
		w = BaseMucJoinDlg::create(this, account);
	}
	bringToFront(w);
}
...
#ifdef YAPSI
	if (!contactList()->accounts().isEmpty()) {
		acc.opt_log = contactList()->accounts().first()->userAccount().opt_log;
	}
#endif
...
#else
	PsiLogger::instance()->log(QString("%1 PsiCon()::statusMenuChanged(%2)").arg(LOG_THIS)
	                           .arg(x));
...
{
	PsiLogger::instance()->log(QString("%1 PsiCon()::setStatusFromDialog(%2, %3, %4)").arg(LOG_THIS)
	                           .arg(s.type()).arg(withPriority).arg(isManualStatus));
...
void PsiCon::checkAccountsEmpty()
{
	while (d->pro.acc.count() == 0) {
		...
		promptUserToCreateAccount();
#else
		// PsiAccount* account = createAccount("ya.ru", "", "",
		//                                     true, "xmpp.yandex.ru", 5222,
...
void PsiCon::processEvent(PsiEvent *e, ActivationType activationType)
{
	...
	if (e->type() == PsiEvent::Auth && !messagingEnabled) {
	...
	if (e->type() == PsiEvent::GroupchatAlert) {
		QPointer<PsiEvent> gcEvent = e;
		if (e->account()) {
			ConferenceBookmark b(QString(),
			                     e->jid().bare(),
			                     true,
			                     QString(),
			                     QString());
			e->account()->actionJoin(b, true);
			if (!gcEvent.isNull()) {
				gcEvent->account()->eventQueue()->dequeue(gcEvent);
				delete gcEvent;
			}
		}
		return;
		...
		QPointer<GroupchatInviteEvent> gcEvent = static_cast<GroupchatInviteEvent*>(e);
		if (e->account()) {
			ConferenceBookmark b(QString(),
			                     gcEvent->groupchat(),
			                     true,
...
bool PsiCon::computerSleeping()
{
	return computerSleeping_;
}

void PsiCon::doSleep()
{
	computerSleeping_ = true;
	PsiLogger::instance()->log(QString("%1 PsiCon()::doSleep()").arg(LOG_THIS));

	setGlobalStatus(Status(Status::Offline, tr("Computer went to sleep"), 0));
#ifdef YAPSI_ACTIVEX_SERVER
	yaOnline()->jabberNotify("disconnected", "false", QString());
#endif
}
...
{
	computerSleeping_ = false;
	PsiLogger::instance()->log(QString("%1 PsiCon()::doWakeup()").arg(LOG_THIS));
...
void PsiCon::restoreSavedChats() const
{
	QTimer::singleShot(1000, d, SLOT(restoreSavedChats()));
}
...
YaHistoryCacheManager* PsiCon::yaHistoryCacheManager() const
{
	return yaHistoryCacheManager_;
}

YaLogEventsManager* PsiCon::yaLogEventsManager() const
{
	return yaLogEventsManager_;
}

YaUnreadMessagesManager* PsiCon::yaUnreadMessagesManager() const
{
	return yaUnreadMessagesManager_;
}
...
YaTransportManager* PsiCon::yaTransportManager() const
{
	return yaTransportManager_;
}

YaMucManager* PsiCon::yaMucManager() const
{
	return yaMucManager_;
}
#endif
