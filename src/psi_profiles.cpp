...
QDomElement UserAccount::toXml(QDomDocument &doc, const QString &tagName)
{
	...
	a.appendChild(textTag(doc, "id", id));
#ifdef YAPSI_ACTIVEX_SERVER
	if (saveable) {
#endif
...
#ifdef YAPSI
	a.appendChild(textTag(doc, "log-on-server", opt_log_on_server));
	a.appendChild(textTag(doc, "jabber-service", jabber_service));
#endif
...
#ifdef YAPSI
	readBoolEntry(a, "log-on-server", &opt_log_on_server);
	readEntry(a, "jabber-service", &jabber_service);
#endif
...
void UserProfile::reset()
{
	...
	prefs.sp[name] = StatusPreset(name, QObject::tr("I am away from my desk.  Leave a message."), XMPP::Status::Away);;
	name = QObject::tr("Showering");
	prefs.sp[name] = StatusPreset(name, QObject::tr("I'm in the shower.  You'll have to wait for me to get out."), XMPP::Status::Away);
	name = QObject::tr("Eating");
	prefs.sp[name] = StatusPreset(name, QObject::tr("Out eating.  Mmmm.. food."), XMPP::Status::Away);
	name = QObject::tr("Sleep");
	prefs.sp[name] = StatusPreset(name, QObject::tr("Sleep is good.  Zzzzz"),XMPP::Status::DND);
	name = QObject::tr("Work");
	prefs.sp[name] = StatusPreset(name, QObject::tr("Can't chat.  Gotta work."), XMPP::Status::DND);
	name = QObject::tr("Air");
	prefs.sp[name] = StatusPreset(name, QObject::tr("Stepping out to get some fresh air."), XMPP::Status::Away);
	name = QObject::tr("Movie");
	prefs.sp[name] = StatusPreset(name, QObject::tr("Out to a movie.  Is that OK with you?"), XMPP::Status::Away);
	name = QObject::tr("Secret");
	prefs.sp[name] = StatusPreset(name, QObject::tr("I'm not available right now and that's all you need to know."), XMPP::Status::XA);
	name = QObject::tr("Out for the night");
	prefs.sp[name] = StatusPreset(name, QObject::tr("Out for the night."), XMPP::Status::Away);
	name = QObject::tr("Greece");
	prefs.sp[name] = StatusPreset(name, QObject::tr("I have gone to a far away place.  I will be back someday!"), XMPP::Status::XA);
#endif
...
#ifdef YAPSI
	prefs.onevent[eSubscribe] = ApplicationInfo::resourcesDir() + "/sound/subscribe.wav";
	prefs.onevent[eUnsubscribe] = ApplicationInfo::resourcesDir() + "/sound/unsubscribe.wav";
#endif
...
#ifdef YAPSI
	{
		// also check yapsi.qss
		QString defaultFamily = "Arial";
		QFont::insertSubstitutions(defaultFamily, QStringList()
		                           << "Sans Serif"
		                           << "MS Shell Dlg 2"
		                          );
		QFont::insertSubstitutions("Tahoma", QStringList()
		                           << "Sans Serif"
		                           << "MS Shell Dlg 2"
		                          );
		...
		prefs.font[fChat] = font.toString();
	}

	prefs.alertOpenChats = true;

	prefs.useEmoticons = true;
	prefs.emoticons = QStringList("yaemomidget");

	prefs.ppIsOn        = false;
	prefs.ppOnline      = false;
	prefs.ppOffline     = false;
	prefs.ppStatus      = false;
	prefs.ppMessage     = false;
	prefs.ppChat        = false;
	prefs.ppHeadline    = false;
	prefs.ppFile        = false;

	prefs.popupMsgs      = true;
	prefs.popupChats     = true;
	prefs.popupHeadlines = true;
	prefs.popupFiles     = true;
	prefs.noAwayPopup    = true;
	prefs.noUnlistedPopup = true;
	...
	// we want to get the mood on each and every login
	prefs.autoVCardOnLogin = true;
#endif
}
...
#ifdef YAPSI
			p_onevent.appendChild(textTag(doc, "subscribe", prefs.onevent[eSubscribe]));
			p_onevent.appendChild(textTag(doc, "unsubscribe", prefs.onevent[eUnsubscribe]));
#endif
		}
	}
...
#ifdef YAPSI
				readEntry(tag, "subscribe", &prefs.onevent[eSubscribe]);
				readEntry(tag, "unsubscribe", &prefs.onevent[eUnsubscribe]);
#endif
			}
		}
