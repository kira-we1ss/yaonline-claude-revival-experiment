...
static const int PDD_DISABLED_ERROR = -11;
static const int PING_TIMEOUT_ERROR = -12;
...
struct HostPortOverride {
	HostPortOverride(QString _host, int _port, UserAccount::SSLFlag _ssl, bool _needPlainTextAuth, bool _needEncryption, bool _needNoEncryption, QString _jabberService)
		: host(_host)
		, port(_port)
		, ssl(_ssl)
		, needPlainTextAuth(_needPlainTextAuth)
		, needEncryption(_needEncryption)
		, needNoEncryption(_needNoEncryption)
		, jabberService(_jabberService)
	{}
	HostPortOverride()
		: port(-1)
		, ssl(UserAccount::SSL_No)
		, needPlainTextAuth(false)
	{}

	QString host;
	int port;
	UserAccount::SSLFlag ssl;
	bool needPlainTextAuth;
	bool needEncryption;
	bool needNoEncryption;
	QString jabberService;
};

static QMap<QString, HostPortOverride> hostPortOverride()
{
	static QMap<QString, HostPortOverride> data;
	if (data.isEmpty()) {
		data["ya.ru"]           = HostPortOverride("xmpp.yandex.ru",      5222, UserAccount::SSL_Yes, true,  false, false, "yandex");
		// data["jabber.ru"]       = HostPortOverride("jabber.ru",           5222, UserAccount::SSL_Yes, false, false, false, "jabber");
		data["yandex-team.ru"]  = HostPortOverride("xmpp.yandex-team.ru", 5222, UserAccount::SSL_Yes, false, false, false, "jabber");
		data["yandex-team.com.ua"]  = HostPortOverride("xmpp.yandex-team.ru", 5222, UserAccount::SSL_Yes, false, false, false, "jabber");
		data["gmail.com"]       = HostPortOverride("talk.google.com",     5222, UserAccount::SSL_Yes, true,  true,  false, "gtalk");
		data["livejournal.com"] = HostPortOverride("livejournal.com",     5222, UserAccount::SSL_No,  false, false, true,  "lj");
		data["qip.ru"]          = HostPortOverride("webim.qip.ru",        5222, UserAccount::SSL_Yes, false, false, false, "qip");
	}
...
public:
	...
	QList<GroupchatContact*> groupchatContacts_;
	...
	bool noPopup(ActivationType activationType) const
	{
		...
		if (lastManualStatus().isAvailable()) {
			if (lastManualStatus().type() == XMPP::Status::DND)
				return true;
			if ((lastManualStatus().type() == XMPP::Status::Away || lastManualStatus().type() == XMPP::Status::XA) && option.noAwayPopup)
				return true;
		}
...
#ifdef YAPSI
		connect(contact, SIGNAL(moodChanged(const QString&, const QString&)), account, SLOT(moodChanged(const QString&, const QString&)));
#endif
...
#ifdef YAPSI_ACTIVEX_SERVER
		if (account->isOnlineAccount() && psi->contactList()->accountsLoaded()) {
			psi->yaOnline()->incomingStanza(s);
		}
#endif
	}
...
public:
	...
	QTimer* groupOfflineStorageMessagesTimer_;
	QList<MessageEvent*> offlineStorageMessages_;

	bool showToasterOnConnect;
	bool showToasterOnError;
	QString lastErrorMessage;
	QTimer* resetConnectionToastersTimer_;
	QTimer* pendingConnectionErrorsTimer_;

	enum ConnectionErrorToasterErrorType {
		CETET_GenericError = 0,
		CETET_AuthError = 1
	};
	struct PendingConnectionError {
		QDateTime dateTime;
		QString error;
		ConnectionErrorToasterErrorType errorType;
	};
	QList<PendingConnectionError> pendingConnectionErrors_;

	void resetConnectionToasters()
	{
		showToasterOnConnect = false;
		showToasterOnError = true;
		resetConnectionToastersTimer_->stop();
	}

	void requestLogHistoryOnServerStatus()
	{
		if (account->isYaAccount() || account->isYaPddAccount()) {
			Message m("history.ya.ru");
			m.setBody("i");
			account->dj_sendMessage(m, false);

			bool isOnlineAccount = psi->contactList()->yaServerHistoryAccount() == account;
			if (isOnlineAccount) {
				foreach(PsiAccount* acc, account->psi()->contactList()->accounts()) {
					if (acc->isYaAccount() || acc->isYaPddAccount()) {
						continue;
					}

					m.setTwin(acc->jid().bare());
					account->dj_sendMessage(m, false);
				}
			}
		}
	...
	void requestMoodSwitch()
	{
		if (account->isOnlineAccount()) {
			JT_YaMoodSwitch* task = new JT_YaMoodSwitch(client->rootTask());
			connect(task, SIGNAL(finished()), SLOT(moodSwitchFinished()));
			task->get();
			task->go(true);
		}
	}

	void requestOfflineEmailsMaxLast()
	{
		if (account->isOnlineAccount()) {
			JT_YaMaxLast* task = new JT_YaMaxLast(client->rootTask());
			connect(task, SIGNAL(finished()), SLOT(offlineEmailsMaxLastFinished()));
			task->get();
			task->go(true);
		}
	}

	void showSuccessfullyConnectedToaster()
	{
		requestLogHistoryOnServerStatus();
		requestMoodSwitch();
		requestOfflineEmailsMaxLast();

#ifdef YAPSI_ACTIVEX_SERVER
		if (psi->contactList()->accountsLoaded()) {
			QString p1 = "true";
			if (account->isOnlineAccount()) {
				psi->yaOnline()->jabberNotify("connected", p1);
				psi->yaOnline()->jabberNotify("auth", p1);
				psi->yaOnline()->jabberNotify("online", p1);

				psi->yaOnline()->doOnlineConnected();
			}
			else {
				psi->yaOnline()->jabberNotify("online-non-main", p1);
			}
			psi->yaOnline()->setStatus(psi->lastLoggedInStatusType());
		}
#endif

		if (showToasterOnConnect) {
			if (!pendingConnectionErrors_.isEmpty()) {
				qWarning("Connection Successful(%s), suppressing toasters", qPrintable(account->jid().bare()));
				pendingConnectionErrors_.clear();
				showToasterOnConnect = false;
			}
		}

		if (showToasterOnConnect) {
			qWarning("Connection Successful(%s)", qPrintable(account->jid().bare()));
#ifdef YAPSI_ACTIVEX_SERVER
			if (psi && psi->contactList()->accountsLoaded() && psi->yaOnline()) {
				psi->yaOnline()->showSuccessfullyConnectedToaster(account);
			}
#endif
			PsiNotifier::instance()->showSuccessfullyConnectedToaster(account);
		}
		showToasterOnConnect = false;
		lastErrorMessage = QString();
	}

	void showConnectionErrorToaster(const QString& error, ConnectionErrorToasterErrorType errorType)
	{
		Q_ASSERT(!error.isEmpty());
		qWarning("Connection Error(%s: %s)", qPrintable(account->jid().bare()), qPrintable(error));
		...
		showToasterOnConnect = true;
		showToasterOnError = false;
		resetConnectionToastersTimer_->start();

		PendingConnectionError err;
		err.dateTime = QDateTime::currentDateTime();
		err.error = error;
		err.errorType = errorType;

		pendingConnectionErrors_.clear();
		pendingConnectionErrors_ << err;
		pendingConnectionErrorsTimer_->start();

		if (errorType == CETET_AuthError) {
			pendingConnectionError();
		}
	}

private slots:
	void pendingConnectionError()
	{
		if (pendingConnectionErrors_.empty())
			return;
		PendingConnectionError error = pendingConnectionErrors_.takeFirst();

#ifdef YAPSI_ACTIVEX_SERVER
		if (psi && psi->contactList()->accountsLoaded() && psi->yaOnline()) {
			psi->yaOnline()->showConnectionErrorToaster(account, error.error, error.errorType);
		}
#endif
		PsiNotifier::instance()->showConnectionErrorToaster(account, error.error);
	}

	void resetConnectionToastersTimerTimeout()
	{
		LOG_TRACE;
		resetConnectionToasters();
		account->clearCurrentConnectionError();
	}

	void moodSwitchFinished()
	{
		JT_YaMoodSwitch* task = static_cast<JT_YaMoodSwitch*>(sender());
		if (task && task->success()) {
			PsiOptions::instance()->setOption("options.ya.publish-mood-on-yaru", task->enabled());
		}
	}

	void offlineEmailsMaxLastFinished()
	{
		JT_YaMaxLast* task = static_cast<JT_YaMaxLast*>(sender());
		if (task && task->success()) {
			PsiOptions::instance()->setOption("options.ya.offline-emails-max-last", task->maxLast());
		}
	}

	void groupOfflineStorageMessages()
	{
		qWarning("groupOfflineStorageMessages(%s)", qPrintable(account->jid().full()));
		if (offlineStorageMessages_.isEmpty())
			return;
		QHash<QString, QList<MessageEvent*> > contacts;

		int totalMessageCount = 0;
		foreach(MessageEvent* me, offlineStorageMessages_) {
			const Message &m = me->message();

			QString j = m.from().bare().toLower();
			if (!contacts.contains(j))
				contacts[j] = QList<MessageEvent*>();
			contacts[j] << me;
			++totalMessageCount;
		}

		if (contacts.count() > 3) {
			qWarning("GroupOfflineMessages(1): %d messages from %d contacts", totalMessageCount, contacts.count());
			XMPP::Message fakeMessage(account->jid());
			fakeMessage.setType("chat");
			fakeMessage.setFrom(offlineStorageMessages_.first()->message().from());

			QHashIterator<QString, QList<MessageEvent*> > it(contacts);
			while (it.hasNext()) {
				it.next();

				foreach(MessageEvent* e, it.value()) {
					offlineStorageMessages_.removeAll(e);
					eventQueue->enqueue(e);
				}
				account->openChat(it.key(), FromXml);
				account->addPendingMessagesForJid(it.key());
			}

			MessageEvent* me = new MessageEvent(fakeMessage, account);
			me->setOriginLocal(false);
			me->setMultiContactCount(contacts.count());
			me->setForcedDescription(QObject::tr("%n messages", 0, totalMessageCount));
			account->handleEvent(me, IncomingStanza);
		}
		else {
			QHashIterator<QString, QList<MessageEvent*> > it(contacts);
			while (it.hasNext()) {
				it.next();
				if (it.value().count() > 1) {
					qWarning("GroupOfflineMessages(2): %d messages from %s contact", it.value().count(), qPrintable(it.key()));
					XMPP::Message fakeMessage(account->jid());
					fakeMessage.setType("chat");
					fakeMessage.setFrom(it.key());
					...
					MessageEvent* me = new MessageEvent(fakeMessage, account);
					me->setOriginLocal(false);
					me->setForcedDescription(QObject::tr("%n messages", 0, it.value().count()));
					account->handleEvent(me, IncomingStanza);
				}
				else {
					qWarning("GroupOfflineMessages(3): %d message from %s contact", it.value().count(), qPrintable(it.key()));
					XMPP::Message message = it.value().first()->message();
					message.setSpooled(false); // ensure that it won't be passed through groupOfflineStorageMessages() again

					MessageEvent* me = new MessageEvent(message, account);
					me->setOriginLocal(false);
					account->handleEvent(me, IncomingStanza);
				}
			}
		...
		qDeleteAll(offlineStorageMessages_);
		offlineStorageMessages_.clear();
	}
#endif // ifdef YAPSI
...
public slots:
	void startReconnect()
	{
		...
		if (reconnectInfrequently_) {
			data.delay = (resetConnectionToastersTimer_->interval() / 1000) + 10;
			data.timeout = 30;
		}

		PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::Private::startReconnect(); delay = %3, timeout = %4").arg(LOG_THIS)
		                           .arg(account->name())
		                           .arg(data.delay)
		                           .arg(data.timeout));
	...
	: QObject(parent)
{
	...
	presenceSent = false;
#ifdef YAPSI
	d->isYaPddAccount = false;
	d->askingForVCard = false;
	d->disableAutoConnect = false;

	d->resetConnectionToastersTimer_ = new QTimer(this);
	d->resetConnectionToastersTimer_->setSingleShot(true);
	d->resetConnectionToastersTimer_->setInterval(3 * 60 * 60 * 1000); // 3 hours
	connect(d->resetConnectionToastersTimer_, SIGNAL(timeout()), d, SLOT(resetConnectionToastersTimerTimeout()));

	d->pendingConnectionErrorsTimer_ = new QTimer(this);
	d->pendingConnectionErrorsTimer_->setSingleShot(true);
	d->pendingConnectionErrorsTimer_->setInterval(30 * 1000); // 30 seconds
	connect(d->pendingConnectionErrorsTimer_, SIGNAL(timeout()), d, SLOT(pendingConnectionError()));

	d->groupOfflineStorageMessagesTimer_ = new QTimer(this);
	d->groupOfflineStorageMessagesTimer_->setSingleShot(true);
	d->groupOfflineStorageMessagesTimer_->setInterval(3 * 1000); // 3 seconds
	connect(d->groupOfflineStorageMessagesTimer_, SIGNAL(timeout()), d, SLOT(groupOfflineStorageMessages()));

	connect(psi()->contactList()->yaPddManager(), SIGNAL(domainDataChanged(const QString&)), SLOT(domainDataChanged(const QString&)));
	...
	d->deliveryConfirmationManager = new DeliveryConfirmationManager(this);
	connect(d->deliveryConfirmationManager, SIGNAL(serverPingTimeout()), SLOT(serverPingTimeout()));
	...
	connect(d->bookmarkManager, SIGNAL(conferencesChanged(const QList<ConferenceBookmark>&)), SLOT(conferencesChanged()));
...
PsiAccount::~PsiAccount()
{
	...
	if (d->contactList->accountsLoaded()) {
		logout(true);
	}
	QString str = name();
...
{
#ifdef YAPSI
	d->psi->getToggles(&d->realAcc.tog_offline, &d->realAcc.tog_away, &d->realAcc.tog_agents, &d->realAcc.tog_hidden, &d->realAcc.tog_self);

	// save the roster and pgp key bindings
	d->realAcc.roster.clear();
	d->realAcc.keybind.clear();
	...
	for(UserListItem *u; (u = it.current()); ++it) {
		...
		if(!u->publicKeyID().isEmpty())
			d->realAcc.keybind.set(u->jid().full(), u->publicKeyID());
	}
...
DeliveryConfirmationManager* PsiAccount::deliveryConfirmationManager() const
{
	return d->deliveryConfirmationManager;
}
...
{
	QString result = QString::number(rand() % 10);
	if (length > 1)
		result += generateRandomString(length - 1);
	return result;
}
#endif
...
void PsiAccount::setUserAccount(const UserAccount &_acc)
{
	...
	// generate new random string only when account settings change
	// this could help to get rid of old sessions quicker when
	// reconnecting and the server haven't already disconnected old
	// session
	acc.resource = QString("YaOnline-%1.%2-%3")
	               .arg(YAPSI_VERSION)
	               .arg(YAPSI_REVISION)
	               .arg(generateRandomString(4));
...
#ifdef YAPSI
	{
		...
		if (hostPortOverride().contains(ajid.host())) {
			if (acc.jabber_service.isEmpty()) {
				HostPortOverride hpo = hostPortOverride()[ajid.host()];
				acc.jabber_service = hpo.jabberService;
				d->realAcc.jabber_service = hpo.jabberService;

				if (hpo.jabberService != "jabber") {
					acc.opt_host = false;
					acc.host = "";
					acc.port = 5222;
					d->realAcc.opt_host = false;
					d->realAcc.host = "";
					d->realAcc.port = 5222;
				}
			}
...

		{
			...
			QMapIterator<QString, HostPortOverride> it(hostPortOverride());
			while (it.hasNext()) {
				it.next();
				if (it.value().jabberService == acc.jabber_service) {
					validService = true;
					break;
				}
			}
			...
			if (!validService) {
				acc.jabber_service = "jabber";
				d->realAcc.jabber_service = "jabber";
			}
		}

#ifdef YAPSI_ACTIVEX_SERVER
		if (isOnlineAccount()) {
			acc.jabber_service = "yandex";
			d->realAcc.jabber_service = "yandex";
		}
#endif

		bool useHostPortOverride = !acc.jabber_service.isEmpty() && acc.jabber_service != "jabber";

		if (useHostPortOverride) {
			Q_ASSERT(acc.jabber_service != "jabber");
			bool found = false;
			HostPortOverride hpo;
			QMapIterator<QString, HostPortOverride> it(hostPortOverride());
			while (it.hasNext()) {
				it.next();
				if (it.value().jabberService == acc.jabber_service) {
					...
					if (!acc.jid.contains("@")) {
						ajid.setDomain(it.key());
						ajid.setNode(acc.jid);
						acc.jid = ajid.full();
					}
					break;
				}
			...
			if (hostPortOverride().contains(ajid.host())) {
				hpo = hostPortOverride()[ajid.host()];
				found = true;
			}
#endif
			...
			if (found) {
				...
				if (acc.ssl != UserAccount::SSL_No || hpo.needEncryption) {
					if (acc.port == 5223 || acc.port == 443)
						acc.ssl = UserAccount::SSL_Legacy;
					else
						acc.ssl = UserAccount::SSL_Yes;
				}

				if (hpo.needNoEncryption) {
					acc.ssl = UserAccount::SSL_No;
				}

#ifdef YAPSI_ACTIVEX_SERVER
				if (isOnlineAccount() && d->psi->contactList()->accountsLoaded()) {
					acc.opt_ignoreSSLWarnings = false;
					acc.ssl = d->psi->yaOnline()->useSsl() ? UserAccount::SSL_Yes : UserAccount::SSL_No;
				}
#endif
			}
		}

		// one more pass, for cases where d->realAcc.jid is domain-less.
		acc.jid = Ya::yaRuAliasing(acc.jid);
	}
#endif
...
#ifdef YAPSI
		autoLogin &= !d->acc.pass.isEmpty() || (id() == "FACEBOOK");
#endif
		if (autoLogin) {
...
#ifdef YAPSI
	QTimer::singleShot(0, this, SLOT(checkLoginPrerequisites()));
#else
	continueLogin();
#endif
}
...
void PsiAccount::checkLoginPrerequisites()
{
	...
	if (!isYaAccount() && !psi()->contactList()->yaPddManager()->knownDomain(domain)) {
		PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::login(); requesting pdd status for '%3'").arg(LOG_THIS)
		                           .arg(name())
		                           .arg(domain));
		psi()->contactList()->yaPddManager()->requestDomainData(domain);
		return;
	}
...
void PsiAccount::domainDataChanged(const QString& domain)
{
	if (domain == d->nextJid.domain()) {
		PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::domainDataChanged(); isActive = %3, isConnected = %4, isDisconnecting = %5").arg(LOG_THIS).arg(name())
		                           .arg(isActive()).arg(isConnected()).arg(isDisconnecting));

		if (isActive() && !isConnected() && !isDisconnecting) {
			continueLogin();
		}
	}
}
...
void PsiAccount::continueLogin()
{
	PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::continueLogin()").arg(LOG_THIS)
	                           .arg(name()));
	...
	PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::login(); useHost = %3; host = %4; port = %5").arg(LOG_THIS)
	                           .arg(name())
	                           .arg(useHost).arg(host).arg(port));
	...
	if(d->acc.proxy_index > 0) {
		...
		PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::login(); pi.type = %3").arg(LOG_THIS)
		                           .arg(name())
		                           .arg(pi.type));
	...
	PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::login(); legacy_ssl_probe = %3; ssl = %4; allow_plain = %5; compress = %6").arg(LOG_THIS)
	                           .arg(name())
	                           .arg(d->acc.legacy_ssl_probe).arg(d->acc.ssl).arg(d->acc.allow_plain).arg(d->acc.opt_compress));
...
#ifdef YAPSI
	d->stream->setAllowXFacebookPlatform(id() == "FACEBOOK");
	// d->stream->setAllowXFacebookPlatform(true);
#endif
...
{
	PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::forceDisconnect()").arg(LOG_THIS)
	                           .arg(name()));
...
void PsiAccount::tls_handshaken()
{
...
#ifdef YAPSI
	d->acc.opt_ignoreSSLWarnings = true;
#endif
	if(r != QCA::TLS::Valid && !d->acc.opt_ignoreSSLWarnings) {
		...
		d->currentConnectionError = tr("The %1 certificate failed the authenticity test. Reason: %1.").arg(str);
		d->currentConnectionErrorCondition = -1;
		emit connectionError(d->currentConnectionError);

#ifdef YAPSI_ACTIVEX_SERVER
		if (isOnlineAccount() && d->psi->contactList()->accountsLoaded()) {
			d->psi->yaOnline()->jabberNotify("disconnected", "false", d->currentConnectionError);
		}
#endif
		// ONLINE-2890
		d->showConnectionErrorToaster(d->currentConnectionError, Private::CETET_GenericError);
#else
...
void PsiAccount::cs_authenticated()
{
	if (d->conn.isNull() || d->stream.isNull()) {
		PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::cs_authenticated(); d->conn = %3; d->stream = %4").arg(LOG_THIS)
		                           .arg(name())
		                           .arg(d->conn.isNull())
		                           .arg(d->stream.isNull()));
		cs_error(RECONNECT_TIMEOUT_ERROR);
...
void PsiAccount::cs_error(int err)
{
	...
	if (!d || !isActive() || !d->client)
		return;

	getErrorInfo(err, d->conn, d->stream, d->tlsHandler, &str, &reconn, &d->disableAutoConnect, &isAuthError, &isTemporaryAuthFailure);
	...
	if(err == PDD_DISABLED_ERROR) {
		d->currentConnectionError = tr("Specified domain is not PDD-enabled.");
		d->currentConnectionErrorCondition = -1;

		d->disableAutoConnect = true;
		isAuthError = false;
		isTemporaryAuthFailure = false;
	}
	if(err == PING_TIMEOUT_ERROR) {
		d->currentConnectionError = tr("Server ping timeout.");
		d->currentConnectionErrorCondition = -1;
	...
	if (isOnlineAccount() && d->psi->contactList()->accountsLoaded()) {
		...
		if (err == XMPP::ClientStream::ErrAuth) {
			...
			d->psi->yaOnline()->jabberNotify("conflict");
		}
		else {
			d->psi->yaOnline()->jabberNotify("disconnected", p1, p2);
		}
	}
#endif
	...
	if (err != RECONNECT_TIMEOUT_ERROR) {
		if (isAuthError) {
			d->showToasterOnError = true;
		}
		Private::ConnectionErrorToasterErrorType errorType = isAuthError ?
		        Private::CETET_AuthError : Private::CETET_GenericError;
		d->showConnectionErrorToaster(d->currentConnectionError, errorType);
	}
#endif
...
void PsiAccount::conferencesChanged()
{
	QList<GroupchatContact*> foundContacts;
	foreach(ConferenceBookmark c, d->bookmarkManager->conferences()) {
		bool found = false;
		GCMainDlg* groupchat = findDialog<GCMainDlg*>(c.jid());
		XMPP::Status::Type active = XMPP::Status::GC_Favorited;
		if (groupchat && !groupchat->isQuitting()) {
			groupchat->setBookmarkName(c.name());

			active = groupchat->isConnected() ?
			         XMPP::Status::GC_Active :
			         XMPP::Status::GC_Inactive;
		}

		foreach(GroupchatContact* contact, d->groupchatContacts_) {
			if (contact->bookmark().jid() == c.jid()) {
				foundContacts << contact;
				contact->setBookmark(c);
		...
		if (!found) {
			GroupchatContact* contact = new GroupchatContact(this);
			contact->setBookmark(c);
			contact->setActive(active);

			d->groupchatContacts_ << contact;
			foundContacts << contact;
	...
	foreach(GroupchatContact* contact, d->groupchatContacts_) {
		if (!foundContacts.contains(contact)) {
			d->groupchatContacts_.removeAll(contact);
			delete contact;
		}
	}
}
...
{
	psi()->contactUpdatesManager()->contactAdded(this, r.jid());
...
void PsiAccount::client_rosterItemRemoved(const RosterItem &r)
{
	...
	psi()->contactUpdatesManager()->contactRemoveFinished(this, u->jid());
...
void PsiAccount::client_resourceAvailable(const Jid &j, const Resource &r)
{
	...
	if(notifyOnlineOk && doPopup && !d->blockTransportPopupList->find(j, popupType == PopupOnline) && !d->noPopup(IncomingStanza)) {
		...
		PsiNotifier::instance()->popup(this, pt, j, r, u);
#endif
	}
	...
	if (d->client && d->client->jid().compare(j, true) && isYaAccount()) {
		...
		if (d->mood != r.status().status() && r.status().status() != d->loginStatus.status()) {
			...
			// safeguard from duplicate mood changes
			foreach(PsiAccount* account, psi()->contactList()->enabledAccounts()) {
				XMPP::Status s = account->d->lastManualStatus();
				s.setStatus(d->mood);
				account->d->setManualStatus(s);
				account->d->loginStatus = s;
			}
...
#endif
		PsiNotifier::instance()->popup(this, PsiPopup::AlertOffline, j, r, u);
	}
}
...
{
	if (_m.error().condition != XMPP::Stanza::Error::UndefinedCondition) {
#ifdef GROUPCHAT
		...
		if (psi()->yaTransportManager()->processIncomingMessage(_m)) {
			return;
		}
	}

#ifdef GROUPCHAT
	if (!_m.mucInvites().isEmpty() && GCMainDlg::mucEnabled()) {
		MUCInvite i = _m.mucInvites().first();
		...
		foreach(int id, GlobalEventQueue::instance()->ids()) {
			...
			if (event->type() == PsiEvent::GroupchatInvite) {
				...
				if (inviteEvent &&
				    inviteEvent->jid().compare(i.from(), false) &&
				    inviteEvent->groupchat().compare(_m.from(), false))
				{
		...
		if (createNewInvite) {
			GroupchatInviteEvent* event = new GroupchatInviteEvent(i.from(), _m.from(),
			        i.reason(), _m.mucPassword(), this);
	...
	if (!_m.lastMailNotify().isNull()) {
	...
	else if (_m.from().compare("lastmail.ya.ru", false)) {
		return;
	}
	else if (_m.from().compare("mail.ya.ru", false)) {
		return;
	}
		...
		if (!_m.twin().isEmpty()) {
			acc = 0;
			XMPP::Jid twinJid(_m.twin());
			foreach(PsiAccount* a, psi()->contactList()->accounts()) {
				if (a->jid().compare(twinJid, false)) {
					acc = a;
					break;
				}
			}
		}
		QDomElement e = _m.getExtension("wch:status");
		Q_ASSERT(!e.isNull());
		Q_ASSERT(e.tagName() == "x");
		if (!e.isNull() && acc) {
			acc->d->realAcc.opt_log_on_server = e.text() == "1";
			acc->d->acc.opt_log_on_server = e.text() == "1";
			emit acc->updatedAccount();
		}
		return;
	}
#endif
...
#ifndef YAPSI
	messagingEnabled = EventDlg::messagingEnabled();
#endif
	if (!messagingEnabled) {
		m.setType("chat");
	}
...
void PsiAccount::client_subscription(const Jid &j, const QString &str, const QString& nick)
{
	...
	if (psi()->contactUpdatesManager()->notifier()->filterContactAuth(this, j, str)) {
		return;
	}
	// psi()->contactUpdatesManager()->contactAuth(this, j, str);
...
#ifdef YAPSI
/**
 * Returns true if this is an account on ya.ru.
 */
...
bool PsiAccount::isYaPddAccount() const
{
	return d->isYaPddAccount;
}

bool PsiAccount::isOnlineAccount() const
{
#ifdef YAPSI_ACTIVEX_SERVER
	return !d->acc.saveable;
#else
	return (isYaAccount() || isYaPddAccount()) && d && d->contactList && d->contactList->defaultAccount() == this;
#endif
}

bool PsiAccount::connectionEnabled() const
{
#ifdef YAPSI_ACTIVEX_SERVER
	if (!d->psi->contactList()->accountsLoaded()) {
		return false;
	}
	return isOnlineAccount() || d->psi->yaOnline()->onlinePasswordVerified();
#else
	return true;
#endif
}
...
void PsiAccount::serverPingTimeout()
{
	PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::serverPingTimeout() isAvailable = %3").arg(LOG_THIS)
	                           .arg(name())
	                           .arg(isAvailable()));

	if (isAvailable()) {
		cs_error(PING_TIMEOUT_ERROR);
	}
}
...
{
#ifdef YAPSI
	bool gcPrivateChat = findGCContact(jid);
#endif
...
void PsiAccount::groupchatEventCreated(PsiEvent* event)
{
	Q_ASSERT(event);
	handleEvent(event, IncomingStanza);
}

void PsiAccount::actionJoin(const Jid& mucJid, const QString& password, bool connectImmediately)
{
	ConferenceBookmark bookmark(QString(), mucJid, false, QString(), password);
	actionJoin(bookmark, connectImmediately);
}

void PsiAccount::actionJoin(const ConferenceBookmark& bookmark, bool connectImmediately)
{
#ifdef GROUPCHAT
	BaseMucJoinDlg* w = (BaseMucJoinDlg*)psi()->dialogFind("YaMucJoinDlg");
	if (!w) {
		w = BaseMucJoinDlg::create(psi(), this);
	}
	...
	w->setNick(bookmark.nick().isEmpty() ? nick() : bookmark.nick());
	w->setPassword(bookmark.password());

	bool doBringToFront = !connectImmediately;
	if (connectImmediately) {
		doBringToFront = !w->doJoinBookmark();
	}
...
{
	PsiLogger::instance()->log(QString("%1 PsiAccount(%2)::simulateRosterOffline()").arg(LOG_THIS)
	                           .arg(name()));
...
#ifdef YAPSI
	bool gcPrivateChat = findGCContact(j);
...
{
#ifdef YAPSI
	QStringList fileNames = FileUtil::getOpenAnyFileNames(QApplication::activeWindow());
	if (!fileNames.isEmpty()) {
		openChat(j, UserPassiveAction);
		YaChatDlg* dlg = dynamic_cast<YaChatDlg*>(ensureChatDlg(j));
		if (dlg) {
			dlg->uploadFiles(fileNames);
		}
	}
	return;
#endif
...
void PsiAccount::dj_auth(const Jid &j)
{
	...
	if (psi()->contactUpdatesManager()->notifier()->isRegisteredJid(j.bare())) {
		dj_rename(j, psi()->contactUpdatesManager()->notifier()->desiredName(j.bare()));
	}
#else
	psi()->contactUpdatesManager()->contactAuthorized(this, j);
	d->client->sendSubscription(j, "subscribed");
...
void PsiAccount::handleEvent(PsiEvent* e, ActivationType activationType)
{
	...
	if (activationType != FromXml) {
		   ...
		   )
		{
			...
			if (e->type() == PsiEvent::Mood) {
				...
				found = psi()->yaHistoryCacheManager()->hasMood(this, e->from(), moodEvent->mood());
			}
#endif
	...
	if(e->type() == PsiEvent::Message) {
		...
		// we're handling message errors in deliveryConfirmationManager()->processIncomingMessage()
		Q_ASSERT(m.type() != "error");
		...
		// message from offline storage
		if (putToQueue && m.spooled()) {
			qWarning("adding spooled message %s %s", qPrintable(jid().full()), qPrintable(m.from().bare()));
			d->offlineStorageMessages_ << me;
			d->groupOfflineStorageMessagesTimer_->start();
			return;
		}
	}
		...
		if (yaChat && activationType == IncomingStanza) {
			MoodEvent* moodEvent = static_cast<MoodEvent*>(e);
			if (!psi()->yaHistoryCacheManager()->hasMood(this, e->from(), moodEvent->mood())) {
	...
	else if (e->type() == PsiEvent::GroupchatAlert) {
		soundType = eChat2;
		putToQueue = true;

		GCMainDlg* c = findDialog<GCMainDlg*>(e->jid().userHost());
		if (c && activationType == IncomingStanza) {
			if (c->isActiveTab()) {
				putToQueue = false;
			}
		}
	}
	else if (e->type() == PsiEvent::GroupchatInvite) {
		soundType = eSubscribe;
		putToQueue = true;
...
#endif
		PsiNotifier::instance()->popup(this, popupType, j, r, u, e);
		emit startBounce();
	}
...
#ifdef YAPSI_ACTIVEX_SERVER
	if (!psi()->restoringSavedChats() && activationType == UserAction && d->psi->contactList()->accountsLoaded()) {
...
{
#ifdef YAPSI
	if (d->acc.opt_log_on_server) {
		logEventOnServer(j, e, this);
	}
#endif
...
#ifdef YAPSI
void PsiAccount::logEventOnServer(const Jid &j, PsiEvent *e, const PsiAccount* account)
{
	psi()->yaLogEventsManager()->logEventOnServer(j, e, account);
}
#endif
...
void PsiAccount::openGroupChat(const Jid &j, ActivationType activationType)
{
	...
	GCMainDlg *w = GCMainDlg::create(j, this, d->tabManager);
	connect(w, SIGNAL(gcStateChanged()), SLOT(conferencesChanged()));
	connect(w, SIGNAL(eventCreated(PsiEvent*)), SLOT(groupchatEventCreated(PsiEvent*)));
...
bool PsiAccount::groupChatJoin(const QString &host, const QString &room, const QString &nick, const QString& pass, bool nohistory)
{
	if (nohistory)
		...
		s.setXSigned("");
		return d->client->groupChatJoin(host, room, nick, pass,
		                                d->options->getOption("options.muc.context.maxchars").toInt(),
		                                d->options->getOption("options.muc.context.maxstanzas").toInt(),
		                                d->options->getOption("options.muc.context.seconds").toInt(),
		                                s);
	}
}
...
void PsiAccount::client_groupChatJoined(const Jid &j)
{
	...
	BaseMucJoinDlg *w = findDialog<BaseMucJoinDlg*>(j);
	if(!w) {
		return;
	}
	...
	m = findDialog<GCMainDlg*>(Jid(j.userHost()));
	if (m) {
...
QDateTime PsiAccount::reconnectScheduledAt() const
{
	return d->reconnectScheduledAt_;
}
#endif
...
#ifdef YAPSI
void PsiAccount::moodChanged(const QString& resource, const QString& mood)
{
	...
	bool moodSet = !contact->getMood(resource).isNull();
	QString jid = contact->jid().withResource(resource).full();
	PsiLogger::instance()->log(QString("PsiAccount::moodChanged(%1, %2, %3)")
	                           .arg(jid).arg(mood).arg(moodSet));

	if (moodSet) {
		if (d->lastTimeMoodChanged_.contains(jid)) {
			contact->startDelayedMoodUpdate(moodUpdateTimeoutSecs - d->lastTimeMoodChanged_[jid].secsTo(currentDateTime));
			PsiLogger::instance()->log(QString("\tdelaying update"));
			return;
		}

		d->lastTimeMoodChanged_[jid] = currentDateTime;
	}

	contact->setMood(resource, mood);

	if (!moodSet) {
		PsiLogger::instance()->log(QString("\t!moodSet"));
		return;
	}

	if (mood.trimmed().isEmpty()) {
		PsiLogger::instance()->log(QString("\tmood empty"));
		return;
	}

	if (!contact->moodNotificationsEnabled()) {
		PsiLogger::instance()->log(QString("\t!moodNotificationsEnabled"));
		return;
	}

	PsiLogger::instance()->log(QString("\tcreating event"));
...
bool PsiAccount::addPendingMessagesForJid(const XMPP::Jid& jid)
{
	bool found = false;
	foreach(TabDlg* tabDlg, psi()->tabManager()->tabSets()) {
		TabbableWidget* tab = tabDlg->getTabPointer(this, jid.full());
