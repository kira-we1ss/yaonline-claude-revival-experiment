...
QString TextUtil::loadText(const QString& fileName)
{
	QString text;
...
QString TextUtil::linkify(const QString &in)
{
	...
	for(int n = 0; n < (int)out.length(); ++n) {
		...
		static QString xmppPrefix("xmpp:");
		static QString httpPrefix("http://");
		static QString httpsPrefix("https://");
		static QString ftpPrefix("ftp://");
		static QString newsPrefix("news://");
		static QString ed2kPrefix("ed2k://");
		static QString wwwDotPrefix("www.");
		static QString ftpDotPrefix("ftp.");
		static QString atPrefix("@");
		if(linkify_pmatch(out, n, xmppPrefix)) {
			n += 5;
			isUrl = true;
			href = QLatin1String("");
		}
		else if(linkify_pmatch(out, n, httpPrefix)) {
			n += 7;
			isUrl = true;
			href = QLatin1String("");
		}
		else if(linkify_pmatch(out, n, httpsPrefix)) {
			n += 8;
			isUrl = true;
			href = QLatin1String("");
		}
		else if(linkify_pmatch(out, n, ftpPrefix)) {
			n += 6;
			isUrl = true;
			href = QLatin1String("");
		}
		else if(linkify_pmatch(out, n, newsPrefix)) {
			n += 7;
			isUrl = true;
			href = QLatin1String("");
		}
		else if (linkify_pmatch(out, n, ed2kPrefix)) {
			n += 7;
			isUrl = true;
			href = QLatin1String("");
		}
		else if(linkify_pmatch(out, n, wwwDotPrefix)) {
			isUrl = true;
			href = QLatin1String("http://");
		}
		else if(linkify_pmatch(out, n, ftpDotPrefix)) {
			isUrl = true;
			href = QLatin1String("ftp://");
		}
		else if(linkify_pmatch(out, n, atPrefix)) {
			isEmail = true;
			href = QLatin1String("mailto:");
		}
		...
		if(isUrl) {
			...
			static QMap<QChar, QChar> openingBracket;
			...
			static QString htmlDangerous("\"\'`<>");
			static QString htmlQuot("&quot;");
			static QString htmlApos("&apos;");
			static QString htmlGt("&gt;");
			static QString htmlLt("&lt;");
			for(x2 = n; x2 < (int)out.length(); ++x2) {
				if (out.at(x2).isSpace()
				    || linkify_isOneOf(out.at(x2), htmlDangerous)
				    || linkify_pmatch(out, x2, htmlQuot)
				    || linkify_pmatch(out, x2, htmlApos)
				    || linkify_pmatch(out, x2, htmlGt)
				    || linkify_pmatch(out, x2, htmlLt))
				{
					break;
				}
			...
			int cutoff;
			static QString unwantedPunctuation("!?,.()[]{}<>\"");
			static QString closingBrackets(")]}");
			for(cutoff = pre.length()-1; cutoff >= 0; --cutoff) {
				if(!linkify_isOneOf(pre.at(cutoff), unwantedPunctuation))
					break;
				if(linkify_isOneOf(pre.at(cutoff), closingBrackets)
			...
			static QString validEmailChars("_.-");
			for(; x1 >= 0; --x1) {
				if(!linkify_isOneOf(out.at(x1), validEmailChars) && !out.at(x1).isLetterOrNumber())
					break;
			}
			++x1;
			...
			for(; x2 < (int)out.length(); ++x2) {
				if(!linkify_isOneOf(out.at(x2), validEmailChars) && !out.at(x2).isLetterOrNumber())
					break;
			}
