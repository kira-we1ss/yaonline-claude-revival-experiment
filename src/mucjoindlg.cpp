...
BaseMucJoinDlg::BaseMucJoinDlg()
	: BaseMucJoinDlgBaseClass()
{
...
BaseMucJoinDlg* BaseMucJoinDlg::create(PsiCon* controller, PsiAccount* defaultAccount)
{
	BaseMucJoinDlg* result;
#ifdef YAPSI
	result = new YaMucJoinDlg(controller, defaultAccount);
	result->init();
#else
	result = new MUCJoinDlg(controller, defaultAccount);
#endif
	return result;
}
...
#ifdef YAPSI
	PsiAccount* acc = psi->contactList()->yandexTeamAccount();
	if (acc && acc->isAvailable()) {
		ui_.cb_ident->setAccount(acc);
		updateIdentity(acc);
		ui_.le_host->setText(QString("conference.%1").arg(acc->jid().domain()));
	}
	else {
		ui_.le_host->setText("conference.ya.ru");
		ui_.le_room->setText("yaru");
	}

	if (psi->contactList()->defaultAccount()) {
		ui_.le_nick->setText(psi->contactList()->defaultAccount()->nick());
	}
#endif
...
void MUCJoinDlg::setAccount(PsiAccount* account)
{
	ui_.cb_ident->setAccount(account);
