...
bool PsiSelfContact::shouldBeVisible() const
{
	...
	return false;
#else
	return PsiContact::shouldBeVisible() || contactList()->showSelf() || userListItem().userResourceList().count() > 1;
#endif
}

ContactListItemMenu* PsiSelfContact::contextMenu(ContactListModel* model)
{
	Q_UNUSED(model);
