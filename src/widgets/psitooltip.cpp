...
QPoint ToolTipPosition::calculateTipPosition(const QWidget* label, const QSize& size) const
{
	Q_UNUSED(size);
...
void PsiToolTip::doShowText(const QPoint &pos, const QString &text, const QWidget *w)
{
	...
	if (PsiTipLabel::instance() && moveAndUpdateTipLabel(PsiTipLabel::instance(), text)) {
		updateTipLabel(PsiTipLabel::instance(), text, static_cast<ToolTipPosition*>(calc));
		return;
	}

	installPsiToolTipFont();
	QFrame *label = createTipLabel(text, QApplication::desktop()->screen(calc->getScreenNumber()));
	label->move(calc->calculateTipPosition(label, label->size()));
...
bool PsiToolTip::preventAnimation() const
{
	return PsiTipLabel::instance() != 0;
}
...
void PsiToolTip::updateTipLabel(PsiTipLabel* label, const QString& text, const ToolTipPosition* pos)
{
	Q_UNUSED(text);

	// fancy moving tooltip effect
	if (pos)
		label->move(pos->calculateTipPosition(label, label->sizeHint()));
}
...
bool PsiToolTip::isVisible() const
{
	return PsiTipLabel::instance() &&
	       PsiTipLabel::instance()->isVisible();
}

bool PsiToolTip::preventShow()
{
	return false;
}
