...
void XmlConsole::dumpRingbuf()
{
	...
	foreach (PsiAccount::xmlRingElem el, buf) {
		dumpXml(el.time, el.xml, el.type != PsiAccount::RingXmlOut);
	}
...
{
	dumpXml(QDateTime::currentDateTime(), str, true);
}

void XmlConsole::client_xmlOutgoing(const QString &str)
{
	dumpXml(QDateTime::currentDateTime(), str, false);
}

void XmlConsole::dumpXml(const QDateTime& dateTime, const QString& xml, bool incoming)
{
	if (filtered(xml))
		return;
...
void XmlPrompt::transmitMalformedXmlConfirmation(const QString& id, bool confirmed)
{
	if (confirmed) {
		transmitHelper(id);
	}
...
void XmlPrompt::transmitHelper(const QString& str)
{
	textReady(str);
	close();
}
