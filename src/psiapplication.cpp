...
: QApplication(argc, argv, GUIenabled)
{
	...
	// we can immediately handle app quit requests if we patch Qt with
	//
	...
	//     void exiting(int);
	//
	if (metaObject()->indexOfSignal("exiting(int)") != -1)
		connect(this, SIGNAL(exiting(int)), SLOT(appExiting(int)));
}
...
void PsiApplication::appAboutToQuit()
{
	PsiLogger::instance()->log("PsiApplication::appAboutToQuit()");
}

void PsiApplication::appExiting(int code)
{
	PsiLogger::instance()->log(QString("PsiApplication::appExiting(): %1").arg(code));
}
