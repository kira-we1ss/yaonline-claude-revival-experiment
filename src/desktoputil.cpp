...
bool DesktopUtil::openUrl(const QString &url)
{
	...
	PsiLogger::instance()->log(QString("DesktopUtil::openUrl(%1)").arg(url));
#ifdef YAPSI_ACTIVEX_SERVER
...
bool DesktopUtil::openYaUrl(const QString& url)
{
	if (yaFilterEmail(url))
		return true;
	PsiLogger::instance()->log(QString("DesktopUtil::openYaUrl(%1)").arg(url));
#ifdef YAPSI_ACTIVEX_SERVER
...
{
	PsiLogger::instance()->log(QString("DesktopUtil::openEMail(%1)").arg(email));
#ifdef YAPSI_ACTIVEX_SERVER
