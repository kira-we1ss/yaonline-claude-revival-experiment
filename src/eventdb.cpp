...
EDBFlatFile::File *EDBFlatFile::ensureFile(const PsiAccount* account, const Jid &j)
{
	...
	if(!i) {
		...
		i = new File(account, Jid(j.userHost()));
...
bool EDBFlatFile::File::removeAccountHistoryDir(const PsiAccount* account, bool removeNonYaHistoryOnly)
{
	...
	bool result = false;
	QString accountPath = JIDUtil::encode(account->jid().userHost()).toLower();
...
{
	QDir accountHistory(ApplicationInfo::historyDir() + "/" + JIDUtil::encode(account->jid().userHost()).toLower());
	if (!accountHistory.exists()) {
		QDir history(ApplicationInfo::historyDir());
		history.mkdir(JIDUtil::encode(account->jid().userHost()).toLower());
	}
...
QString EDBFlatFile::File::jidToFileName(const PsiAccount* account, const XMPP::Jid &j)
{
	Q_ASSERT(account);
	if (account) {
		return accountHistoryDir(account) + "/" + JIDUtil::encode(j.userHost()).toLower() + ".history";
	}
