...
QString XMPP::Message::twin() const
{
	return d->twin;
}

void XMPP::Message::setTwin(const QString& twin)
{
	d->twin = twin;
}

QString XMPP::Message::yaMessageId() const
{
	return d->yaMessageId;
}

void XMPP::Message::setYaMessageId(const QString& yaMessageId)
{
...
Stanza Message::toStanza(Stream *stream) const
{
	...
	if (!d->twin.isEmpty()) {
		s.setAttribute("twin", d->twin);
	}

	if (!d->yaMessageId.isEmpty()) {
		QDomElement yaMessageId = s.createTextElement("yandex:message:id", "x", d->yaMessageId);
...
bool Message::fromStanza(const Stanza &s, int timeZoneOffset)
{
	...
	if (root.hasAttribute("twin")) {
		d->twin = root.attribute("twin");
	}

	t = root.elementsByTagNameNS("yandex:message:id", "x").item(0).toElement();
	if (!t.isNull()) {
		d->yaMessageId = t.text();
	}
	XMLHelper::removeNodes(root, t);
#endif
...
void Status::setType(Status::Type _type)
{
	...
	switch(_type) {
		...
		case GC_Active: show = "gc-active"; break;
		case GC_Inactive: show = "gc-inactive"; break;
		case GC_Favorited: show = "gc-fav"; break;
#endif
		default: break;
	}
	setShow(show);
...
void Status::setType(QString stat)
{
	...
	else if (stat == "gc-active")
		setType(XMPP::Status::GC_Active);
	else if (stat == "gc-inactive")
		setType(XMPP::Status::GC_Inactive);
	else if (stat == "gc-fav")
		setType(XMPP::Status::GC_Favorited);
#endif
	else
		setType(XMPP::Status::Away);
}
...
Status::Type Status::type() const
{
	...
	if (!isAvailable()) {
		...
		if (s == "away")
			...
			type = XMPP::Status::GC_Active;
		else if (s == "gc-inactive")
			type = XMPP::Status::GC_Inactive;
		else if (s == "gc-fav")
			type = XMPP::Status::GC_Favorited;
#endif
	}
	return type;
}
...
QString Status::typeString() const
{
	...
	switch(type()) {
		...
		case GC_Active: stat = "gc-active"; break;
		case GC_Inactive: stat = "gc-inactive"; break;
		case GC_Favorited: stat = "gc-fav"; break;
#endif
		default: stat = "away";
	}
	return stat;
...
QList<int> XMPP::Status::getMUCStatuses() const
{
	if (hasMUCStatus()) {
		QList<int> result;
		result << mucStatus();
...
#ifdef YAPSI
const QString & Status::notifyValue() const
{
	return v_notifyValue;
}

void Status::setNotifyValue(const QString &value)
{
	v_notifyValue = value;
}
#endif
