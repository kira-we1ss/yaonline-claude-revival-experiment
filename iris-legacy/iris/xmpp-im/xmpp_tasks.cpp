...
#ifdef YAPSI
		else if(i.tagName() == "x-notify" && i.attribute("xmlns") == "yandex:informer") {
			QDomElement t;
			bool found;
			t = findSubTag(i, "value", &found);
			if (found)
				p.setNotifyValue(tagContent(t));
			else
				p.setNotifyValue(QString());
		}
#endif
	}
...
const Jid & JT_DiscoItems::jid() const
{
	return d->jid;
}
...
// JT_YaLogMessage
//----------------------------------------------------------------------------
JT_YaLogMessage::JT_YaLogMessage(Task *parent)
:Task(parent)
{
}
...
void JT_YaLogMessage::log(bool originLocal, const YaDateTime& timeStamp, const XMPP::Jid& accountJid, const Message &message)
{
	originLocal_ = originLocal;
...
void JT_YaLogMessage::onGo()
{
	if (originLocal_ && m_.from().isNull()) {
		m_.setFrom(selfJid_);
	}

	Stanza s = m_.toStanza(&(client()->stream()));
	QDomElement e = oldStyleNS(s.element());

	QDomElement iq = createIQ(doc(), "set", "history.ya.ru", id());
	QDomElement query = doc()->createElement("query");
	query.setAttribute("xmlns", "yandex:history:foreign");
	query.setAttribute("action", originLocal_ ? "send" : "recv");
	query.setAttribute("twin", selfJid_.bare());
	query.setAttribute("timestamp", timeStamp_.toYaTime_t());
	query.appendChild(e);
	iq.appendChild(query);
...
bool JT_YaLogMessage::take(const QDomElement &x)
{
	if (!iqVerify(x, "history.ya.ru", id()) && !iqVerify(x, "ya.ru", id()))
		return false;
...
// JT_YaRemoveHistory
//----------------------------------------------------------------------------
JT_YaRemoveHistory::JT_YaRemoveHistory(Task *parent)
: Task(parent)
{
}

JT_YaRemoveHistory::~JT_YaRemoveHistory()
{
}

void JT_YaRemoveHistory::remove(const XMPP::Jid& owner)
{
	selfJid_ = owner;
}

void JT_YaRemoveHistory::remove(const XMPP::Jid& owner, const XMPP::Jid& twin)
{
	selfJid_ = owner;
	twin_ = twin;
}

void JT_YaRemoveHistory::remove(const XMPP::Jid& owner, const XMPP::Jid& twin, const XMPP::Jid& contact)
{
	selfJid_ = owner;
	twin_ = twin;
	contact_ = contact;
...
void JT_YaRemoveHistory::onGo()
{
	Q_ASSERT(!selfJid_.isNull() && selfJid_.isValid());

	QDomElement iq = createIQ(doc(), "set", "history.ya.ru", id());
	QDomElement query = doc()->createElement("query");
	query.setAttribute("xmlns", "yandex:history:delete");
	query.setAttribute("owner", selfJid_.bare());
	if (!twin_.isNull() && !twin_.compare(selfJid_))
		query.setAttribute("twin", twin_.bare());
	if (!contact_.isNull())
		query.setAttribute("opp", contact_.bare());
...
bool JT_YaRemoveHistory::take(const QDomElement &x)
{
	if (!iqVerify(x, "history.ya.ru", id()))
		return false;
...
// JT_YaRemoveHistory
//----------------------------------------------------------------------------
JT_YaRetrieveHistory::JT_YaRetrieveHistory(Task *parent)
	: Task(parent)
	, messageCount_(-1)
	, checkUnread_(false)
{
...
JT_YaRetrieveHistory::~JT_YaRetrieveHistory()
{
}

void JT_YaRetrieveHistory::retrieve(const XMPP::Jid& contact, int messageCount, YaDateTime startTime)
{
	contact_ = contact;
...
{
	if (!checkUnread_) {
		Q_ASSERT(!contact_.isNull() && contact_.isValid());
	}

	int paginatorMax = messageCount_;

	QDomElement iq = createIQ(doc(), "get", "history.ya.ru", id());
	QDomElement query = doc()->createElement("retrieve");
	query.setAttribute("xmlns", "yandex:history:retrieve");
	if (!startTime_.isNull()) {
		query.setAttribute("start", startTime_.toYaIsoTime());
	}
	if (!checkUnread_) {
		query.setAttribute("with", contact_.bare());
		// query.setAttribute("order_by", "asc");
	}
	else {
		query.setAttribute("unread", "1");
		paginatorMax = 100;
	}

	QDomElement set = doc()->createElement("set");
	set.setAttribute("xmlns", "yandex:history:paginator");
	set.appendChild(textTag(doc(), "max", QString::number(paginatorMax)));
...
bool JT_YaRetrieveHistory::take(const QDomElement &x)
{
	if (!iqVerify(x, "history.ya.ru", id()))
		return false;
	...
	if (x.attribute("type") == "result") {
		bool found;
		QDomElement q = findSubTag(x, "chat", &found);
		Q_ASSERT(q.attribute("xmlns") == "yandex:history:retrieve");
		for(QDomNode n = q.firstChild(); !n.isNull(); n = n.nextSibling()) {
			QDomElement e = n.toElement();
			if( e.isNull() )
				continue;

			if ( e.tagName() == "to" || e.tagName() == "from" ) {
				JT_YaRetrieveHistory::Chat chat;
				chat.timeStamp = YaDateTime::fromYaIsoTime(e.attribute("time"));
				chat.body = XMLHelper::subTagText(e, "body");
				chat.originLocal = e.tagName() == "to";
				chat.jid = e.attribute("user");

				if (chat.timeStamp.isNull()) {
					qWarning("JT_YaRetrieveHistory::take(%s) '%s' '%s'", qPrintable(q.attribute("with")), qPrintable(e.attribute("time")), qPrintable(chat.timeStamp.toString()));
				}
				messages_ << chat;
			}
		}
...
const XMPP::Jid& JT_YaRetrieveHistory::contact() const
{
	return contact_;
}

const QList<JT_YaRetrieveHistory::Chat>& JT_YaRetrieveHistory::messages() const
{
	return messages_;
}
...
// JT_YaMessageRead
//----------------------------------------------------------------------------
JT_YaMessageRead::JT_YaMessageRead(Task* parent)
	: Task(parent)
{
}


JT_YaMessageRead::~JT_YaMessageRead()
{
}


void JT_YaMessageRead::messageRead(const XMPP::Jid& contact, const YaDateTime& timeStamp)
{
	contact_ = contact;
	timeStamp_ = timeStamp;
}
...
JT_YaMoodSwitch::~JT_YaMoodSwitch()
{
}

void JT_YaMoodSwitch::get()
{
	isGet_ = true;
	enabled_ = false;
}

void JT_YaMoodSwitch::set(bool enabled)
{
	isGet_ = false;
	enabled_ = enabled;
}

bool JT_YaMoodSwitch::enabled() const
{
	Q_ASSERT(isGet_);
	return enabled_;
}

void JT_YaMoodSwitch::onGo()
{
	QDomElement iq = createIQ(doc(), isGet_ ? "get" : "set", "", id());
	QDomElement query = doc()->createElement("query");
	query.setAttribute("xmlns", "yandex:mood-switch");
	if (!isGet_) {
		QDomElement moodSettings = doc()->createElement("mood-settings");
		moodSettings.setAttribute("enabled", enabled_ ? "1" : "0");
		query.appendChild(moodSettings);
	}
	iq.appendChild(query);
...
bool JT_YaMoodSwitch::take(const QDomElement& x)
{
	if (!iqVerify(x, client()->host(), id()))
		return false;
	...
	if (x.attribute("type") == "result") {
		if (isGet_) {
			bool found;
			QDomElement q = findSubTag(x, "query", &found);
			Q_ASSERT(found);
			Q_ASSERT(q.attribute("xmlns") == "yandex:mood-switch");

			QDomElement tag = findSubTag(q, "mood-settings", &found);
			Q_ASSERT(found);
			if (found) {
				enabled_ = tag.attribute("enabled") == "1";
			}
		}
...
// JT_YaMaxLast
//----------------------------------------------------------------------------
JT_YaMaxLast::JT_YaMaxLast(Task* parent)
	: Task(parent)
	, isGet_(false)
	, maxLast_(-1)
{
...
JT_YaMaxLast::~JT_YaMaxLast()
{
}

void JT_YaMaxLast::get()
{
	isGet_ = true;
	maxLast_ = -1;
}

void JT_YaMaxLast::set(int maxLast)
{
	isGet_ = false;
	maxLast_ = maxLast;
}

int JT_YaMaxLast::maxLast() const
{
	Q_ASSERT(isGet_);
	return maxLast_;
}

void JT_YaMaxLast::onGo()
{
	QDomElement iq = createIQ(doc(), isGet_ ? "get" : "set", "", id());
	QDomElement query = doc()->createElement("query");
	query.setAttribute("xmlns", "yandex:max-last");
	if (!isGet_) {
		if (maxLast_ > -1) {
			query.appendChild(textTag(doc(), "max-last", QString::number(maxLast_)));
		}
		else {
			query.appendChild(doc()->createElement("max-last"));
		}
	}
	iq.appendChild(query);
...
bool JT_YaMaxLast::take(const QDomElement& x)
{
	if (!iqVerify(x, client()->host(), id()))
		return false;
	...
	if (x.attribute("type") == "result") {
		if (isGet_) {
			...
			QDomElement q = findSubTag(x, "query", &found);
			Q_ASSERT(found);
			Q_ASSERT(q.attribute("xmlns") == "yandex:max-last");

			QString maxLast = XMLHelper::subTagText(q, "max-last");
			if (maxLast.isEmpty()) {
				maxLast_ = -1;
			}
			else {
				maxLast_ = maxLast.toInt();
			}
		}
