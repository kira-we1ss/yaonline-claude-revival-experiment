...
QString S5BManager::genUniqueSID(const Jid &peer) const
{
	...
	do {
		...
		for(int i = 0; i < 4; ++i) {
			...
			for(int n = 0; n < 4; ++n) {
				sid.append(QString::number((word >> (n * 4)) & 0xf, 16));
			}
		}
...
void S5BManager::con_unlink(S5BConnection *c)
{
	...
	delete e->i;
	d->activeList.removeOne(e); delete e;
}
...
void S5BConnector::reset()
{
	...
	delete d->active;
	d->active = 0;
	qDeleteAll(d->itemList); d->itemList.clear();
}
...
void S5BConnector::item_result(bool b)
{
	...
	if(b) {
		...
		d->itemList.removeOne(i); delete i;
...
{
	if(d->mode == 1)
		d->t.setSingleShot(true); d->t.start(15000);
	send(d->iq);
}
