...
*Последнее обновление: 2026-04-17 (Claude: **XEP-0384 OMEMO multi-device carbons работают — свои сообщения из yachat видны на Android-телефоне (Conversations).** Замкнули последний пробел в OMEMO: (а) `iris-legacy/iris/xmpp-im/xmpp_task.cpp iqVerify()` молча отбрасывал IQ-ответы с пустым `from`, когда это ответ на self-PEP запрос (`get kweiss@.../axolotl.devicelist`) — Prosody/стандартные XMPP-серверы по RFC 6120 §10.3.3 имеют право опускать `from` при ответах от имени собственного JID пользователя; из-за этого `PEPGetTask::take()` возвращал false, задача никогда не завершалась, `publishBundle()` fetch-merge continuation висел вечно, и наш device id не попадал в общий devicelist. Исправлено: принимаем пустой `from` если `to` совпадает с нашим локальным JID. (б) После фикса (а) другой клиент (Android Conversations), тоже пишущий свой единственный id, оверрайтнул наш merged-devicelist — сервер прислал PEP push с устаревшим списком без 5802, наш глобальный слушатель переотправлял publishBundle, Android переоверрайтил и так далее — 131 417 строк лога за 40 секунд, 100% CPU, OMEMO откатывался в TLS. Исправления: (б-1) удалён auto-republish branch из `setPepManager()` слушателя — merge происходит один раз на входе в публикацию bundle; (б-2) 10-секундный debounce в `publishBundle()` через `Private::lastPublishBundleMs`. (в) `fetchContactBundles()` для own-JID пропускает наш собственный device id — libsignal не умеет шифровать для самого себя. Результат: `encrypt: SUCCESS — produced 6 <key> entries` на каждое исходящее сообщение (1 для контакта + 5 для наших устройств, включая Android device 554552063 который после первого раунда перешёл из `PreKey` в `SignalMsg` — подтверждение активной сессии); Android видит `[OMEMO] client_messageReceived: from= kweiss@.../Conversations.cEX3` — key-only handshake пришёл обратно на yachat, bidirectional session подтверждён.)*
