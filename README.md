...
*Последнее обновление: 2026-04-17 (Claude: **XEP-0384 OMEMO — исправление 4 ключевых багов.** 1) `YaChatDlg::isEncryptionEnabled()` было захардкожено `return false` — кнопка-замок никогда не активировала шифрование; теперь запрашивает `OmemoManager::isEnabled(jid())`. 2) `ChatDlg::sendMessage()` получил OMEMO-ветку: асинхронный fetch бандлов → `sessionsEstablished` → `encrypt()` → `encryptDone` → `aSend()` с `<encrypted>` элементом через `addExtension()`; plaintext-fallback при неудаче. 3) `OmemoManager::fetchContactBundles()` была заглушкой (немедленно emit sessionsEstablished); теперь реально запрашивает PEP devicelist → bundles → строит Signal X3DH-сессии через `session_builder_process_pre_key_bundle()`. 4) `psiaccount.cpp`: флаг `omemoBundlePublished` в Private struct; bundle публикуется при подтверждении PEP в `serverFeaturesChanged()` и при `sentInitialPresence()`; флаг сбрасывается при каждой новой сессии в `cs_authenticated()`. Сборка чистая, 0 ошибок.)*
