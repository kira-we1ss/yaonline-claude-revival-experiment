...
| Inline image preview для URL'ов с image-расширением | 🔲 отложено | Пробовали: `ChatImageLoader` singleton с QNAM + disk cache + `QTextDocument::ImageResource`; регексп скан `messageText` внутри `YaChatViewDelegate::ensureMessageLayout` и инъекция `<img src=url>` через `doc->addResource(ImageResource, ...)`; async callback с `QPersistentModelIndex` + `emit sizeHintChanged`. Компилируется чисто но в реальности код не выполнялся на MUC-сообщениях (`[ChatImage] scanning messageText` в лог не попадал) — надо глубже трейсить как MUC сообщения проходят через `YaChatViewModel` vs direct-chat путь. Откачено по запросу пользователя |
...
**Inline image preview — попробовали, откатили:** сделали `ChatImageLoader` singleton с shared `QNetworkAccessManager` + disk cache + memory LRU, регексп скан `messageText` внутри `YaChatViewDelegate::ensureMessageLayout`, инъекцию `<img src=url>` через `doc->addResource(QTextDocument::ImageResource, url, placeholder)` с async callback который заменяет placeholder на реальный `QImage` + emit `sizeHintChanged`. Компилируется чисто, но на реальных MUC-сообщениях код-path не выполнялся (`[ChatImage] scanning messageText` в лог не попадал ни разу), видимо MUC сообщения рендерятся каким-то другим путём чем direct-chat через эту же delegate. Откатили по запросу пользователя; файлы удалены, оставил только тему-пикер. Отложено.
