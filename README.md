...
*Последнее обновление: 2026-04-17 (Claude: **XEP-0384 OMEMO 1-на-1 работает сквозь Conversations/Cheogram** — шифрование и расшифровка подтверждены в живой переписке. Ключевые исправления после первого запуска: (а) `sess_load_session` возвращал `SG_ERR_UNKNOWN`=-1000 при отсутствии сессии — по контракту libsignal должен возвращать `0` («не найдено», не ошибка); отрицательное значение короткозамыкало `session_builder_process_pre_key_bundle` на -1000 — именно поэтому «всегда TLS»; (б) crypto callback `omemo_encrypt`/`omemo_decrypt` игнорировал параметр `cipher` и всегда использовал AES-CBC, хотя libsignal просит либо `SG_CIPHER_AES_CTR_NOPADDING` либо `SG_CIPHER_AES_CBC_PKCS5`; (в) реализован `ik_get_identity_key_pair` (был заглушкой `return SG_ERR_UNKNOWN`) через `ec_public_key_serialize`/`ec_private_key_serialize` на живом `ratchet_identity_key_pair`; (г) заменены QCA-обёртки HMAC-SHA256 и SHA-512 на прямой OpenSSL EVP; (д) формат key material = 32 байта (AES-128-ключ ‖ GCM-tag), `<payload>` = только ciphertext (Conversations spec); (е) key-only handshake (16 или 32B, пустой payload) теперь корректно закрывается как успешный session advance без показа fallback-текста; (ж) `encrypt()` использует объединение (PEP devicelist) + (сохранённые сессии) — продолжает работать когда контакт отозвал устройство; (з) `main.cpp`: `std::_Exit(returnValue)` в конце — обход двойного освобождения OpenSSL-атом `ossl_method_store_free` между QCA и прямым libcrypto atexit handlers. Результат: OMEMO 🔒 зелёный → сообщения проходят обе стороны; входящие 32B key material декодируются как `IT WORKS`/`bruh`; исходящие идут в активные сессии; падения при выходе больше нет.)*
